...
return \OCA\OpenRegister\AppHost\Routes::standard(
    [
        ...
        // Both are #[NoAdminRequired] on the controller. The dark route MUST be listed
        // first: its {slug} pattern also matches a "-dark.svg" URL (capturing e.g.
        // "foo-dark"), so if the light route came first it would shadow the dark route
        // and serve "foo-dark.svg" as light-of-app-"foo-dark" (→ app-not-found fallback).
        // Registering dark first makes "{slug}-dark.svg" win for dark URLs, while a plain
        // "{slug}.svg" URL only matches the light route (slugs are kebab-case and never
        // end in "-dark"). Placed before the SPA catch-all; after exports.
