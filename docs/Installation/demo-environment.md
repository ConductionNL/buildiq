...
This page gets a working Buildiq running on your own machine in two commands. You end with application definitions rendered from the registry.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/buildiq/development/buildiq-compose.yaml
```
...
==> installing buildiq <version>
==> apps present: buildiq integriq openregister thematiq
```
...
```bash
curl -s http://localhost:8615/status.php
```
...
| What | Where |
| --- | --- |
| **Buildiq** | [http://localhost:8615/apps/buildiq/](http://localhost:8615/apps/buildiq/) |
| Admin interface | [http://localhost:8615](http://localhost:8615) — `admin` / `admin` |
...
| `buildiq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing buildiq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8615/apps/buildiq/"
...
curl -s -u admin:admin "http://localhost:8615/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f buildiq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f buildiq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f buildiq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
