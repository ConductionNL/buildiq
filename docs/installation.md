...
sidebar_position: 7
title: Installation
description: Install OpenBuild from the Nextcloud app store (or from source) and stand up your first app.
---

# Installation

OpenBuild is a Nextcloud app. Install it like any other Nextcloud app — from the app store, or from source if you're tracking development.
...
- 500 MB free disk space for OpenBuild itself; expect more for the OR-attached files (icons, document attachments).
...
OpenBuild sits on top of [OpenRegister](https://github.com/ConductionNL/openregister) — the foundation app every Conduction product builds on. Install OpenRegister first; OpenBuild will refuse to enable without it.
...
3. Search for `OpenBuild` — install + enable.
4. Navigate to `https://your-nextcloud/index.php/apps/openbuild` to land on the Virtual apps index.
...
```bash
# 1. Clone OpenRegister + OpenBuild into Nextcloud's custom_apps
cd /path/to/nextcloud/custom_apps
git clone https://github.com/ConductionNL/openregister.git
git clone https://github.com/ConductionNL/openbuild.git

# 2. Install PHP + JS dependencies
cd openregister && composer install && npm ci && npm run build
cd ../openbuild   && composer install && npm ci && npm run build

# 3. Enable
occ app:enable openregister
occ app:enable openbuild

# 4. Run the install + post-migration repair chain
occ maintenance:repair
```

The repair chain runs `InitializeSettings` (imports the openbuild register from `lib/Settings/openbuild_register.json` into OR), `MigrateToVersionedModel` (idempotent — short-circuits on a fresh install), `PopulateApplicationPermissions` (gives the deploying admin owner role on any pre-existing Applications), and `SeedApplicationTemplates` (loads the four starter templates into the templates marketplace).

## Restrict OpenBuild to specific groups (optional)

OpenBuild enables to all signed-in users by default. To gate it (REQ-OBRBAC-006):

```bash
occ app:enable --groups=team-alpha,team-beta openbuild
```
...
1. **Top-bar entry** — `OpenBuild` should appear in your Nextcloud top bar.
2. **API surface** — `curl -u admin:admin -H "OCS-APIRequest: true" https://your-nextcloud/index.php/apps/openbuild/api/applications` returns `[]` (no apps yet) or the seeded set.
...
4. **Built-app route** — your new app is reachable at `/apps/openbuild/{slug}` once you publish the development version.

If any of these fail: `occ log:tail` + check the OR repair-step output. Common gotchas:

- OpenRegister not enabled → OpenBuild refuses to enable. Enable OR first.
