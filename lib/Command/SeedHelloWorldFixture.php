		...
		return [
			...
			'pages' => [
				...
				[
					...
					'type' => 'detail',
					'title' => 'Message',
					// ⚠️ THE BODY GRID IS EJECTED ON PURPOSE — AN AUTO-BODY
					// DETAIL PAGE LOSES ITS DATA WIDGET ON A LOST RACE.
					//
					// A `type: detail` page with no widgets/layout takes
					// nc-vue's auto-body path. CnDetailPage fires
					// `Promise.all([fetchObject(), fetchSchema()])`, but
					// `shouldRenderAutoBody` flips as soon as the OBJECT lands
					// (it does not wait for the schema), the watcher
					// materialises the grid EXACTLY ONCE, and
					// `materializeAutoBody()` DROPS the Data widget when
					// `currentSchema` is still null. Nothing rebuilds it when
					// the schema arrives, and `fetchSchema` fails silently, so
					// there is no error state either — the page renders its
					// header and "Related" and simply has no data on it.
					//
					// Measured in CI (job 95207190738, playwright-traces): the
					// object response completed at 16.171 s and the schema at
					// 16.181 s — ten milliseconds apart, object first — and the
					// captured page snapshot has no Data widget. That is the
					// whole of the builder-host "detail page must render the
					// seeded body text" failure. It passes on a developer box
					// because the schema usually wins.
					//
					// Ejecting the default grid takes the explicit-grid path
					// instead (`hasGridLayout` wins over the auto-body), where
					// the widget's schema arrives through CnPageRenderer's
					// read-through detail context and fills in reactively. The
					// shape below is byte-for-byte nc-vue's own
					// `defaultDetailGrid()`, which is also exactly what
					// OpenBuild's edit button writes the moment anyone edits
					// this page — so this changes no pixels, it only removes
					// the race.
					//
					// The underlying nc-vue defect is NOT fixed by this: any
					// other auto-body detail page still has it. Tracked
					...
					'config' => [
						...
						'widgets' => [
							[
								...
								'type' => 'data',
								'title' => 'Data',
								'content' => ['register' => $reg, 'schema' => 'hello-message', 'columns' => 3, 'overrides' => new \stdClass()],
							],
							[
								'id' => 'related',
								'widgetId' => 'related',
