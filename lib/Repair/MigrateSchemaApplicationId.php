 ...
 * `buildiq`. Every schema this app already owns still carries
 * `application = 'openbuild'`, so the pair matches nothing — and
 ...
 *
 * Measured on a live install before this step existed: 17 schemas still under
 * `openbuild` and zero under `buildiq`, and none of their slugs collide — so
 * all 17 move. The gap is fleet-wide, not specific to one app — 231 under
 * `docudesk`, 200 under `openconnector`, 180 under `procest`, 98 under
 * `decidesk`, 21 under `softwarecatalog`, 2 each under `planix`, `larpingapp`
 * and `hrmq`.
 *
 ...
 *
 * @spec exclude No canonical spec covers the `openbuild` -> `buildiq` schema
...
class MigrateSchemaApplicationId implements IRepairStep {
	 ...
	 * @var string
	 */
	public const OLD_APP_ID = 'openbuild';
	 ...
	 * @var string
	 */
	public const NEW_APP_ID = 'buildiq';
	...
	public function getName(): string {
		return 'Move Buildiq schemas onto the buildiq application id';
	}//end getName()
