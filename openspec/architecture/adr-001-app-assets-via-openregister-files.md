...
Buildiq virtual apps need to carry asset payloads alongside their manifest:

- A nav-bar icon (and dark-mode variant) so each published app is recognisable in the Nextcloud top bar (see chain spec [`buildiq-nextcloud-nav`](../changes/)).
...
2. **A separate file store managed by buildiq** — disk path under `data/buildiq/`, or a parallel NC Files folder. Keeps blobs out of the register but forces us to invent a second persistence/permission/export mechanism just for app assets.
...
**All assets that belong to an Buildiq virtual app are stored as files attached to the `application` register record via OpenRegister's files-attached-to-object functionality.** This includes (non-exhaustively) the light/dark nav icons, future per-app logos/banners, document templates, and any user-uploaded blobs surfaced as app features.
...
Buildiq exposes the asset bytes to the rest of Nextcloud (and to anonymous nav-icon fetches) through a thin Buildiq-side endpoint that reads the attached file from OR — never by replicating the bytes into buildiq's own filesystem.
...
**Negative / trade-offs:**
- Adds a hard dependency on OR's files-attached-to-object endpoint being installed and reachable. Mitigated: Buildiq already requires OR (chain spec #1 application-register).
- Nav-icon rendering needs a small buildiq-side endpoint that fans out to OR; we don't get to serve the SVG straight from `/img/`. Mitigated: same controller can cache and apply CSP/MIME headers correctly per request.
...
- **Standalone buildiq file store** — rejected: forces us to reinvent RBAC, export, and lifecycle for a single use case; diverges from the rest of the Conduction fleet which standardises on OR-attached files.
...
- Chain spec `buildiq-nextcloud-nav` (icon picker UX + nav-entry registration; first consumer of this ADR)
- Chain spec `buildiq-exporter` (must bundle OR-attached files into the export tarball)
