...
openbuilt is the low-code application builder where citizen developers and ambtenaren assemble register-based apps from schemas, pages, workflows, and business rules. The chain ships roughly a dozen specs (`openbuilt-application-register`, `openbuilt-page-designer`, `openbuilt-schema-designer`, `workflow-designer`, `business-rules-engine`, `openbuilt-template-catalogue`, `gemma-starter-pack`, `openbuilt-runtime`, `openbuilt-rbac`, `openbuilt-exporter`, `openbuilt-version-snapshots`, `environments-deployment-pipeline`) plus assorted infra concerns.
...
A cross-cutting IA design pass (`/tmp/ia-small5.md`, 2026-05-22) covered openbuilt alongside four sibling apps (financeq, purchaseq, planix, scholiq) and applied the same compression discipline: collapse tier-suffixed and adapter specs into sub-pages/tabs/widgets, demote infrastructure to per-resource tabs, keep top-level menus bounded between 4 and 6.

## Decision

Adopt a **5-menu top-level navigation** for openbuilt:
...
*Rationale:* promoting GEMMA to its own top-level menu would imply openbuilt is a GEMMA tool first and an app builder second, and would force every non-overheid maker to scan past it. Inside Catalog, GEMMA gets prominence via ordering and badging without distorting the IA.
...
|---|---|---|
| `openbuilt-application-register` | menu | Apps |
| `openbuilt-page-designer` | sub-tool | Designers > Page Designer |
| `openbuilt-schema-designer` | sub-tool | Designers > Schema Designer |
| `workflow-designer` | sub-tool | Designers > Workflow Designer |
| `business-rules-engine` | sub-tool | Designers > Business Rules Designer |
| `openbuilt-template-catalogue` | menu | Catalog |
| `gemma-starter-pack` | sub-page | Catalog > GEMMA-starterpack |
| `openbuilt-runtime` | tab | Apps > app > Runtime/Preview (+ global in Beheer) |
| `openbuilt-rbac` | tab | Apps > app > RBAC (+ global in Beheer) |
| `openbuilt-exporter` | tab | Apps > app > Export (+ global in Beheer) |
| `openbuilt-version-snapshots` | tab | Apps > app > Versies (+ snapshot-policy in Beheer) |
...
| GEMMA-starterpack as top-level menu | Implies openbuilt is a GEMMA tool first; non-overheid makers scan past it; one template feed shouldn't shape the IA |
...
- Builds on the fleet IA design pass `ia-small5.md` (2026-05-22) that covered financeq, purchaseq, planix, scholiq, openbuilt with the same compression discipline.
