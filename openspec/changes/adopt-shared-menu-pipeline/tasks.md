...
- [ ] 1.1 Confirm `@conduction/nextcloud-vue`'s installed version exports
...
- [ ] 2.1 In `src/main.js`, import `buildManifest` from
      `@conduction/nextcloud-vue` alongside the existing
      `CnPageRenderer`/`defaultPageTypes`/`registerIcons`/`registerTranslations`
      import.
- [ ] 2.2 Add `import menuLayout from './menu-layout.json'`.
- [ ] 2.3 Keep the existing `require.context('./manifest.d/', false,
...
- [ ] 2.4 Delete the local `mergeManifestFragments` function
      (`src/main.js:69-84`) once `buildManifest` produces an equivalent
      `mergedManifest`.

## 3. Add menu-layout.json

- [ ] 3.1 Create `src/menu-layout.json` with the three ADR-044 keys, all
      empty/no-op initially: `{ "relocations": {}, "removals": [],
      "settingsSection": [] }`.
- [ ] 3.2 Document in a comment (or the file's README convention used by
...
- [ ] 4.1 Diff the resolved `mergedManifest.pages` and `.menu` arrays
...
- [ ] 4.2 Manual smoke test: every existing OpenBuild page (Dashboard, Apps,
      ...
      settings foldout for Features & roadmap, unchanged).
- [ ] 4.3 Run the existing Playwright suite for navigation-adjacent specs
      (`app-nav-entries`) to confirm no route/menu regression.

## 5. Verify

- [ ] 5.1 `npm run lint` and `npm run build` succeed.
- [ ] 5.2 `npm test` (Vitest) passes, including any new snapshot/regression
      test from task 4.1.
