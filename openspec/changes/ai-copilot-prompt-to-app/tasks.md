...
- [ ] 1.1 Add a public `getToolDescriptors(): array` accessor on
...
- [ ] 1.2 Add `lib/Service/Copilot/CopilotPlanValidator.php` — pure class
...
- [ ] 1.3 Add `lib/Service/Copilot/CopilotPromptBuilder.php` — builds the
...
- [ ] 2.1 Add `lib/Service/CopilotService.php` with
...
- [ ] 2.2 In `CopilotService`, implement `plan(string $brief, ?string
...
- [ ] 2.3 In `CopilotService::plan()`, reject a hybrid or unknown `appSlug`
      with `unsupported_target`/`not_found` before any LLM call, and run
      `CopilotPlanValidator` on the parsed plan (REQ-OBAIC-002,
      REQ-OBAIC-005)
- [ ] 2.4 In `CopilotService`, implement `predictManifests(array $plan,
      ...
      (REQ-OBAIC-003)
- [ ] 2.5 In `CopilotService`, implement `execute(array $plan, string
...
- [ ] 2.6 In `CopilotService::execute()`, implement rollback: on any
      ...
      (REQ-OBAIC-004)
- [ ] 2.7 Add owners/editors RBAC in `CopilotService` for plans targeting an
...
- [ ] 2.8 SPDX + `@license`/`@copyright` docblocks and
...
- [ ] 3.1 Add `lib/Controller/CopilotController.php` with `health()`
...
- [ ] 3.2 Register the three routes in `appinfo/routes.php` inside the
...
- [ ] 4.2 Add `src/composables/useCopilot.js` (Vue 2.7 composable) — state
      ...
      (REQ-OBAIC-003)
- [ ] 4.3 Add `@spec` JSDoc tags on the exported functions of
      `src/services/copilot.js` and `src/composables/useCopilot.js`

## 5. Wizard "Generate with AI" (frontend)

- [ ] 5.1 Add `src/dialogs/CopilotGenerateDialog.vue` — standalone `NcModal`
...
- [ ] 5.2 In `src/dialogs/CreateApplicationWizard/Step1Basics.vue`, render
...
- [ ] 6.2 Add `src/components/copilot/CopilotPanel.vue` — chat-style side
      ...
      (REQ-OBAIC-007)
- [ ] 6.3 Mount the panel in `src/views/PageDesignerHost.vue` behind a
...
- [ ] 8.3 Extend `tests/Unit/Service/CopilotServiceTest.php` — predicted
...
- [ ] 8.4 Add `tests/Unit/Controller/CopilotControllerTest.php` — auth
...
- [ ] 10.2 Add `tests/e2e/copilot-panel.spec.ts` — same skip/stub approach
      ...
      (REQ-OBAIC-007)
- [ ] 10.3 Reference the spec in test titles
...
- [ ] 11.1 Add `docs/ai-copilot.md` — what the copilot does, the
...
- [ ] 11.2 Register the page in `docs/sidebars.js` and link it from
