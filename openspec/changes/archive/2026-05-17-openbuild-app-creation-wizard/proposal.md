...
`openbuild-versioning-model` (spec C / ADR-002) splits the runtime model into
...
- **NEW** `lib/Controller/ApplicationCreationController.php` — receives
  `POST /apps/openbuild/api/applications/wizard`; validates the payload; delegates to
...
- `openbuild-application-register`: The index-page "Add Application" entry point now
...
- **Depends on** `openbuild-versioning-model` (spec C) — specifically the
...
- **Out of scope** (covered by sibling specs):
  - Promotion flow — `openbuild-version-promotion`
  - `?_version=` URL routing — `openbuild-version-routing`
  - Detail-page version switcher — `openbuild-app-detail-overview`
