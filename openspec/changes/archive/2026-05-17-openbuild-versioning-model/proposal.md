...
OpenBuild today conflates "the app" and "the version" on a single `Application` row: the
...
step. Three sibling specs (`openbuild-version-promotion`, `openbuild-version-routing`,
`openbuild-app-creation-wizard`) and the detail-page overview spec all build on this
...
- **BREAKING** Split the `Application` schema into two related OR objects:
  ...
  - **NEW** `ApplicationVersion` carries `name` (admin display string), `slug`
    ...
    (per-version OR register name, convention `openbuild-{appSlug}-{versionSlug}`),
...
- **NEW** `MigrateToVersionedModel` repair step — destructive **green-field migration**:
  ...
  (`openbuild-{slug}`) and deletes the Application row, logging one line per deleted app
  (`Migrated-to-versioned-model: dropped Application '<slug>' and register
  'openbuild-<slug>'`). Idempotent: short-circuits when the schema is already in
...
- **MOVED** The existing `on_transition` lifecycle action that upserts the
  `BuiltAppRoute` index on `published` (from chain spec `openbuild-application-register`)
...
- **MAYBE-REMOVED** `lib/Repair/SeedHelloWorld.php` — verified for residual
  ...
  responsibility of the creation wizard (`openbuild-app-creation-wizard`) at install
...
- `openbuild-application-register`: Schema patch — drop `manifest`, `version`, `status`,
...
- `openbuild-version-snapshots`: Snapshot/writeback semantics are **retired**. Remove
...
- **Modified JSON**:
  - `lib/Settings/openbuild_register.json` — split `Application` schema, add new
...
- **OpenRegister dependency** — uses the existing `^v0.2.10` floor declared in
  `lib/Settings/openbuild_register.json` (relations, lifecycle, register-delete API).
- **Out of scope** (covered by sibling specs in the chain):
  - Promotion dialog + backend — `openbuild-version-promotion`
  - `?version=` URL routing + admin gate — `openbuild-version-routing`
  - Creation wizard provisioning Application + N ApplicationVersions + per-version
    registers + Hello World seed — `openbuild-app-creation-wizard`
