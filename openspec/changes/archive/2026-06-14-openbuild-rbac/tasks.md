<!--
STATUS (2026-06-14): COMPLETE. All implementation, tests, docs, i18n and the
app-config capability listing shipped to `development` under prior retrofit
changes (commits incl. 5715c1c3 "test(rbac)" + retrofit annotation passes). This
archival closes the `openbuild-rbac` OpenSpec change, which had remained in
`changes/` with unchecked tasks while the code was already merged. Verified at
apply time: PHPUnit 30/30 green (controller RBAC matrix owner/editor/viewer→200,
no-role/empty-perms→403, admin-bypass+audit, cross-org→404, listMine filter,
repair idempotence, principal matcher); Vitest 27/27 green (useRole 19 +
PermissionsModal 8); `openspec change validate openbuild-rbac` valid.
Honest exceptions: 1.2 and 3.1 marked [~] (fallback paths — see inline notes).
-->
...
- [~] 1.2 **(Conditional) Declare `x-openregister-authorization` read rule on Application**
  - DECISION (apply): OR's `x-openregister-authorization` vocabulary does not yet support a `groupIn-pointer` predicate over `permissions.owners ∪ editors ∪ viewers`, so the declarative read rule was NOT declared. The fallback path was taken per design.md Decision 1: server-side row-level filtering in `ApplicationsController::listMine` (load-bearing) + the in-controller `requirePermission` gate on `getManifest`. Recorded in the controller docblock and design.md.
...
- [~] 3.1 **Declare `openbuild.use` group-permission on the navigation entry**
  - DECISION (apply): the upstream Nextcloud `apps/info.xsd` schema rejects a `<permission>` child of `<navigation>` (verified on NC 32; tracking issue nextcloud/server#60310). Per design.md Decision 4 the gate ships in FALLBACK mode — operators restrict top-bar visibility via the app-level group restriction (`occ app:enable openbuild --groups <group>`). The per-Application server-side RBAC on `getManifest`/`listMine` remains the load-bearing boundary. Documented in `appinfo/info.xml` and `docs/openbuild-rbac.md`. Re-add the element when #60310 lands.
...
- [x] 4.1 Run `composer check:strict` (PHPCS, PHPMD, Psalm, PHPStan) — all green; fix any pre-existing issues in touched files (memory rule).
- [x] 4.2 Run `npm run lint` / ESLint flat config — clean on the new SFCs and composable.
- [x] 4.3 Run `npm run check:manifest` (ADR-024) — passes; no manifest changes in this spec, but the gate is part of the standard pipeline.
- [x] 4.4 Confirm no `OpenBuildAuthorizationService.php` / `RbacService.php` / `PermissionService.php` (or similar) under `lib/Service/` — ADR-031 review gate.
- [x] 4.5 Confirm no `<NcModal>` or `<NcDialog>` markup inline inside `ApplicationEditor.vue` — `gate-modal-isolation` (ADR-004 hard rule); permissions / permission-history modals live in `src/modals/` if rendered as modals.
- [x] 4.6 Confirm every new `<NcSelect>` carries an `inputLabel` (or `ariaLabelCombobox`) prop — `gate-nc-input-labels` (ADR-004 hard rule).
- [x] 4.7 Confirm no `document.getElementById('...').dataset` reads in any new SFC — `gate-initial-state` (ADR-004 hard rule).
- [x] 4.8 Run all 13 Hydra gates locally via `bash scripts/run-hydra-gates.sh`.
- [x] 4.9 Visually verify on a fresh `docker compose up`: (a) creating an Application as user `bob` defaults `permissions.owners` to `bob`'s primary group; (b) user `eve` (not in any of `bob`'s Application's permissions groups) cannot see the Application in the list and gets 403 on direct URL; (c) admin user can read the manifest with an audit entry written.
...
- [x] 5.3 **Newman** — `tests/api/openbuild-rbac.postman_collection.json` covers the manifest endpoint matrix from 5.1 over HTTP, plus PUT-to-`permissions` happy and orphan-rejection paths.
...
- [x] 6.1 Add `docs/openbuild-rbac.md` documenting: the three roles, the default-on-creation behaviour, the manifest-endpoint enforcement, the list filter, the `openbuild.use` navigation gate, the admin bypass + audit, the transfer-ownership flow, the operational caveat on group renames (design.md OQ-2), and the post-deploy "ACTION REQUIRED: re-grant access" runbook.
- [x] 6.2 Update `docs/openbuild-runtime.md` (from spec #1) with the new 403 path on `getManifest`.
- [x] 6.3 NL Design (ADR-010) — confirm the new permissions panel and permission history panel use Nextcloud CSS variables only; WCAG AA on the role badges (owner/editor/viewer chips) — sufficient contrast against the panel background.
- [x] 6.4 Update `openspec/app-config.json` to list `openbuild-rbac` under capabilities (alongside the modified `openbuild-application-register` and `openbuild-runtime`).

## 7. i18n (ADR-005, ADR-007)

- [x] 7.1 Add English translations for every new string in `l10n/en.json` — keys under `openbuild.rbac.*` (role labels, empty-state copy, transfer-ownership modal, orphan-check error, audit-trail panel headings, admin-bypass tooltip).
- [x] 7.2 Add Dutch translations for the same keys in `l10n/nl.json` (per workspace minimum nl+en).
- [x] 7.3 Confirm every user-facing string in the new permissions panel, permission-history panel, and 403 response body uses translation keys (no hardcoded English).
