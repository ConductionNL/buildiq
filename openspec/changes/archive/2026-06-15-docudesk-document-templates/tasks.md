...
note: |
  Code, validation, runtime, dependency-management, l10n, vitest, e2e and Newman
  are BUILT and green (build exit 0; 24/24 hydra gates green; full vitest 702
  passing). Tasks 1.2 and 1.3's *issue-filing* legs are DEFERRED ([~]) — they are
  external Codeberg-issue housekeeping on other repos and explicitly "not a merge
  blocker". Task 1.3's *verification* leg IS done: the Docudesk surface was
  verified against the deployed app (templates index/show/preview + correspondence
  /generate all @NoAdminRequired; format set pinned to CorrespondenceService::
  VALID_FORMATS = ['pdf','docx','html','email']; generate body shape
  { templateId, dataRefs:[{register,schema,id}], options:{format}, filename }
  confirmed). The e2e scenarios are written + @e2e-annotated but test.skip'd under
  the standing Conduction/openbuild#41 builder-UI quarantine (same as every other
  tests/e2e/spec-coverage spec) — behaviour is proven by vitest + Newman.
---

## 1. Manifest declaration + validation

- [x] 1.1 **Define `runtime.documents[]` and validate it app-side**
...
- [x] 2.1 **Implement src/dialogs/DocumentTemplateAttachmentDialog.vue**
...
- [x] 2.2 **Documents section on the application-detail/designer surface**
...
- [x] 3.1 **Implement src/composables/useDocudeskDocument.js — generate + download**
...
- [x] 3.2 **Implement src/components/runtime/DocumentActions.vue + registry entry**
...
- [x] 4.1 **Auto-manage `docudesk` in manifest dependencies[] on save**
...
- [x] 5.1 **Playwright e2e: attach, preview, generate, download, absence**
...
- [x] 5.2 **Newman: pin the Docudesk integration contract**
...
- [x] 5.3 **Quality gates + regression**
  - spec_ref: All
