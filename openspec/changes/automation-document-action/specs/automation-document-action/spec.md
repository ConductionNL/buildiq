...
reference on the triggering object's designated attachment field; `download-link`
— generate a short-lived signed download URL without persisting a file;
`notify` — dispatch a notification referencing the generated document,
requiring `attach` or `download-link` to also be configured (an automation
with only `notify` and neither other mode SHALL be rejected at editor-save
time as incomplete).
