...
OpenBuild is the fleet's citizen-developer app builder ("Compose Nextcloud
...
`src/manifest.json` nav, the `conduction.nl/apps/openbuild` product page
...
- The product page (EN+NL) and `docs/intro.md` claimed **"n8n workflows"** as
  ...
  something OpenBuild implements. Procest workflow attachments
...
- `info.xml` declared no `<app>` dependency despite `src/manifest.json`
  ...
  virtual-app manifest, schema, and object OpenBuild manages is an
  OpenRegister row (ADR-024).
- "Config-over-code" and "fork-free customisation" — the positioning already
  used for OpenBuild on `connext.mdx` — were not reflected in the product
...
| Schema designer | `src/views/SchemaDesigner.vue`, spec `openbuild-schema-designer` |
...
| Template store — curated starters + remote template store | `src/views/TemplateGallery.vue`, `lib/Controller/StoreController.php` (`search()`/`install()`, spec `openbuild-remote-template-store`), `lib/Repair/SeedApplicationTemplates.php` |
...
| First-time-setup / onboarding walkthrough tours | `src/manifest.json` `walkthrough` block (`trigger: "first-visit"`, `openbuild:getting-started` tour), `src/views/WalkthroughDesignerHost.vue`, `WalkthroughDesigner.vue`, `WalkthroughRecorder.vue` |
...
| 8 MCP chat tools (listApps, getAppManifest, createApp, promoteVersion, upsertSchema, upsertPage, addWidget, upsertMenuItem) | `lib/Mcp/OpenBuildToolProvider.php` + one handler class per tool under `lib/Mcp/Handler/` — IDs/names match the product page's `McpToolShelf` exactly |
...
| NL page's dead `docs.conduction.nl/openbuild` link | Real docs deploy topology serves at `openbuild.conduction.nl` (`docs/docusaurus.config.js` `url:`). Corrected to match the EN page. |
...
- Affected files: `openbuild/appinfo/info.xml`;
  `conduction-website/src/pages/apps/openbuild.mdx`;
  `conduction-website/i18n/nl/docusaurus-plugin-content-pages/apps/openbuild.mdx`;
  `openbuild/docs/intro.md`.
