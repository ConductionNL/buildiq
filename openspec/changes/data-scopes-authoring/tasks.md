...
- [ ] 1.1 Create `src/composables/useOrAccessCapabilities.js`: read
...
- [ ] 1.2 Vitest `src/composables/useOrAccessCapabilities.spec.js`:
...
- [ ] 2.1 Create `src/components/schema-editor/AccessEditor.vue`
...
- [ ] 2.2 In the same file export pure converters
...
- [ ] 2.3 Render unrepresentable preserved entries as a read-only
      "managed outside the designer" list inside `AccessEditor.vue`.
- [ ] 2.4 Condition rows: field `NcSelect` fed from a `fieldNames` prop
      (staged FieldEditor field names), operator fixed to `equals` for
      v1, value input offering the `@user.uid` token or a literal.
- [ ] 2.5 Vitest `src/components/schema-editor/AccessEditor.spec.js`:
...
- [ ] 3.1 `src/views/SchemaDesigner.vue`: add
...
- [ ] 3.2 Mount `<AccessEditor>` after `<RelationEditor>` with
      `:access`, `:field-names`, `:read-only`, and `@update:access` →
      `onAccessChange` (same staged-spread pattern as
      `onRelationsChange`).
- [ ] 3.3 Lock-out warning: computed `authorLockedOut` — staged `read`
...
- [ ] 3.4 Production owner gating: computed `accessReadOnly` — active
...
- [ ] 3.5 Vitest `src/views/SchemaDesigner.access.spec.js`:
...
- [ ] 4.1 `src/components/schema-editor/SchemaListPanel.vue`: export a
...
- [ ] 4.2 Vitest `src/components/schema-editor/SchemaListPanel.spec.js`
...
- [ ] 5.1 `tests/e2e/schema-access-scopes.spec.ts` — authoring flow:
      ...
      (REQ-OBDSA-005).
- [ ] 5.2 `tests/e2e/schema-access-scopes-rbac.spec.ts` — multi-user
...
- [ ] 6.1 Add a "Data scopes (row-level access)" section to the schema
...
- [ ] 6.2 `docs/openbuild-rbac.md`: add the boundary paragraph required
