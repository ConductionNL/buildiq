...
- `VersionPromotionService` (through the new `VersionSchemaCarrier`) matches every schema namespaced to the source version
...
- `ApplicationDeletionService` (through the new `VersionSchemaLocator`) also finds a version's schemas by their namespaced
