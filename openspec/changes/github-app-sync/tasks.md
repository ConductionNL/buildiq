...
- [ ] 1.3 Ensure-repo: when `Application.githubRepo` is unset (or a `repo` override targets a new repo), create it via the broker (`POST /user/repos` or `POST /orgs/{org}/repos`) **PUBLIC by default (`"private": false`)** so the shop's anonymous search can discover it — overridable via an optional `visibility` push param (`'public'`|`'private'`, default `'public'`); set the discovery topic `buildiq-app` (`PUT /repos/{owner}/{repo}/topics`), and store `githubRepo` + `githubDefaultBranch` on the Application. (REQ-GHAS-002)
- [ ] 1.4 Port the Git Data API tree-push mechanics from `GitHubPushService` (blob → tree → commit → ref) but route EVERY call through `CredentialBrokerService::request(credentialId, 'buildiq', method, path, headers, body, actingUserId)`; parent the commit on the current default-branch head (fetched via `GET …/git/ref/heads/{branch}`) so push ADDS a commit (non-destructive, never force). (REQ-GHAS-002, REQ-GHAS-005)
...
- [ ] 5.1 PHPUnit `GitHubAppSyncService::push`: serialize via `AppRepoSerializer`; every GitHub call routed through a mocked `CredentialBrokerService::request` (assert no token in Buildiq); unlinked app creates the repo + sets topic `buildiq-app`; commit is parented on the head (non-destructive); `commitSha` stamped on the version; moved-head → `push_conflict`. (REQ-GHAS-002, REQ-GHAS-005)
...
- [ ] 6.1 Update the Buildiq docs with the owner round-trip (link/publish/pull), the non-destructive conflict model (push=new commit, pull=new draft version), the broker-routed auth (token never in Buildiq), and the feature-detection / disabled-publish hint. Note the exporter is unchanged and orthogonal.
