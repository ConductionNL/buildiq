...
- [ ] 1.3 Implement `fetchDescriptor(owner, repo, ref?, actingUserId?)`: fetch the repo's root `buildiq-app.json` via the contents API; validate `owner`/`repo`/`ref` against safe patterns before path interpolation; return the parsed descriptor or a null/unparseable marker (never drop the hit). (REQ-GHSC-002)
...
- [ ] 1.5 Implement `fetchRepoFiles(owner, repo, ref?, actingUserId?)`: return the `path => contents` map (`buildiq-app.json`, `manifest.json`, every `schemas/*.json`) via the contents API (directory listing + per-file fetch, or the git-tree API), shaped for `AppRepoParser::parse`. (REQ-GHSC-006)
