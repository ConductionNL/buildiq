...
- [ ] 5.1 Update the five capability specs (`business-rules-engine`, `settings-and-observability`, `openbuild-template-catalogue`, `docudesk-document-templates`, `app-icon-management`): add this change to their `**OpenSpec changes**` list and set status `in-progress`.
- [ ] 5.2 Run the Hydra mechanical gates + PHP/JS lint and confirm green.
