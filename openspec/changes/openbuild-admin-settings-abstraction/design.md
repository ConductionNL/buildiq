...
1. **Wrong gate.** It gates on the Nextcloud super-admin flag. The entitled principal is the **app's owner group**, which Buildiq already models as `Application.permissions.owners` (default group `admin`, populated by `PopulateApplicationPermissions`, matched by `PermissionResolver` using the `group:<gid>` grammar). Buildiq also already publishes the caller's groups as the `buildiq.currentUserGroups` initial state (DashboardController). NC super-admin and app-owner are different sets.
...
- Reuse existing Buildiq permission primitives end-to-end; invent nothing parallel.
...
This change touches three surfaces: a JSON manifest-schema field (config), nextcloud-vue Vue components (code), and Buildiq PHP (code). Per ADR-032, `mixed` is an anti-pattern, so the choice is (a) one `code` spec treating the schema edit as thin declarative glue, or (b) a 2–3 link chain (`…-manifest` config → `…-renderer` code → `…-owner-signal` code) with `depends_on`.
...
1. Load `buildiq.currentUserGroups` via `loadState('buildiq', 'currentUserGroups', [])` (initial-state, not DOM attributes — hydra initial-state gate).
...
No new permission model, no `PermissionResolver` grammar change. The frontend gate (D3) needs two inputs, both already produced by Buildiq:
- `buildiq.currentUserGroups` — already published by `DashboardController::publishCurrentUserGroups()`.
