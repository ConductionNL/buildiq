...
- [ ] 3.1 `openspec validate openbuild-data-import-wizard --strict` clean; vitest for the composable + wizard (stubbed OR import endpoint: create-schema path, existing-schema path, rollback, non-builder hidden, shared-register excluded); manifest still Ajv-valid; no OpenBuild-side import backend added.
  - **spec_ref**: all
  - **acceptance_criteria**:
