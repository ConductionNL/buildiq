...
- **Spec the posture** — add an explicit CSRF requirement to the `openbuild-exporter` capability so the rule is enforceable by gate review, mirroring the wording already used in `app-override-persistence`.
...
- `openbuild-exporter`: adds a requirement that export submission enforces Nextcloud CSRF protection; the download stream is explicitly exempted (navigation download).
