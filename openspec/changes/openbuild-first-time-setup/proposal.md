...
OpenBuild has adopted the walkthrough contract (ADR-043 — `src/manifest.json:8-27` ships a `walkthrough` block with the `openbuild:getting-started` tour) but NOT the first-time-setup contract (ADR-042): `src/manifest.json` has no `setup` block, while peer apps pipelinq, procest, larpingapp and opencatalogi all ship one. The gap is not cosmetic — it reproduces exactly the failure modes ADR-042 was written for:
...
The nc-vue building blocks named by ADR-042 (`CnWizardDialog`, `fieldsFromSchema`, the manifest v2 `setup` block, `run-action` step type) are the same machinery the sibling change `openbuild-walkthrough-editor` already plans to let OpenBuild users *author* for their virtual apps — but that change edits OTHER apps' setup blocks; OpenBuild's OWN manifest still has none. The builder app should be a first-class example of the contract it teaches.
...
- **Add a `setup` block to `src/manifest.json`** (manifest v2, per ADR-042): `{ enabled: true, version: 1, completionConfigKey: "setup_completed_version", steps: [...] }` with:
  - `info` step — what OpenBuild is, what the wizard will do.
...
- **Gate the walkthrough on setup completion** — the `openbuild:getting-started` tour keeps `trigger: "first-visit"` but the setup wizard (admin-facing) runs first for admins; non-admin users on an unconfigured instance see the standard not-yet-configured state instead of a dead-end tour (exact interplay per the nc-vue `CnAppRoot` phased boot described in ADR-042).
...
- `openbuild-first-time-setup`: the manifest `setup` block (steps, completion key), the admin-only idempotent seed-templates action endpoint, and the seeded/configured gating semantics.

### Modified Capabilities

_None in this repo's existing spec set._ (`settings-and-observability` is consumed unchanged — the config-fields step writes through the existing settings surface; `openbuild-template-catalogue` seeding semantics are reused, not modified.)
...
- Sibling coordination: `openbuild-walkthrough-editor` (edits setup blocks of virtual apps) — no file overlap; this change is OpenBuild's own manifest.
