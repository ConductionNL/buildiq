# openbuild-mcp-surface Specification

**Status**: planned
**Scope**: openbuild
**OpenSpec changes**:
- `openbuild-mcp-adoption` — declares the ADR-063 read-only dialect and removes every app-mutating tool from the MCP surface (kind: code)

## Purpose

Defines what an MCP agent may and may not do with OpenBuild. OpenBuild authors applications, so its
...
### Requirement: OpenBuild exposes no hand-written MCP tool
The system MUST NOT register any `IMcpToolProvider` implementation for app id `openbuild`, and MUST NOT annotate any OpenBuild service method with `#[McpTool]`.

The `OCA\OpenRegister\Mcp\IMcpToolProvider::openbuild` container alias MUST be removed from
`lib/AppInfo/Application.php`, and the class formerly registered under it MUST NOT implement
`IMcpToolProvider`. OpenBuild's entire agent-facing tool surface MUST therefore be derived from
schema declarations.

#### Scenario: The openbuild MCP catalog contains only derived tools
- GIVEN OpenRegister builds the tool catalog for app id `openbuild`
- WHEN an MCP client lists available tools
- THEN every returned tool id MUST match the 3-segment derived form `openbuild.{schema}.{verb}`
- AND no 2-segment hand-written id such as `openbuild.upsertPage` MUST appear
...
Concretely, none of `openbuild.createApp`, `openbuild.promoteVersion`, `openbuild.upsertSchema`,
`openbuild.upsertPage`, `openbuild.addWidget` or `openbuild.upsertMenuItem` MUST be reachable through
MCP, and no `x-openregister-mcp` block in OpenBuild's register MUST declare a `create`, `update` or
...
- GIVEN an agent holds every grant OpenRegister can issue for app id `openbuild`
...
- WHEN the derived catalog for `openbuild` is built
...
- **Performance:** The `openbuild` catalog MUST stay at or below roughly a dozen tools, so tool-selection accuracy and prompt token cost do not degrade (ADR-063 rule 3).
- **Internationalization:** No new user-facing strings; tool `description` prose is agent-facing English. N/A for `nl_NL`/`en_US`.

## Acceptance Criteria

- [ ] No `IMcpToolProvider::openbuild` alias is registered and no class in `openbuild` implements it
- [ ] The `openbuild` catalog contains exactly ten derived read tools and nothing else
- [ ] No `create`/`update`/`delete` tool id exists for `openbuild`
