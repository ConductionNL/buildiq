...
@e2e exclude pure-backend config contract — IAppConfig read/write of
`registry_url`/`registry_token` and the token-never-returned guarantee are
PHPUnit `SettingsService` contracts; the admin form is covered by the
settings-and-observability UI tests.
...
@e2e exclude pure-backend error-path contract — unreachable/invalid/no-registry
branches are PHPUnit service contracts with a mocked HTTP client; no
Playwright-testable surface for transport internals.
...
@e2e exclude security-contract spec — SSRF rejection of private/reserved hosts
is a PHPUnit service contract; no Playwright-testable UI surface for the
network-layer guard.
...
@e2e exclude mixed spec — the resolve→clone backend contract (201 with the new
Application UUID, `templateOrigin` fields, namespaced companion schemas) is a
PHPUnit controller + service contract; the install UI flow is covered by the
template-catalogue-ui Vitest tests and the catalogue Playwright tests.
