# Tasks — OpenBuilt schema-declared notifications

- [x] Add `x-openregister-notifications` to the `exportJob` schema in `lib/Settings/openbuilt_register.json` with `export-succeeded` (transition action `succeeded`) and `export-failed` (transition action `failed`) rules
- [x] Add `x-openregister-notifications` to the `ApplicationVersion` schema in `lib/Settings/openbuilt_register.json` with `version-published` (transition action `published`) and `version-archived` (transition action `archived`) rules
...
- [x] Validate that `lib/Settings/openbuilt_register.json` is still well-formed JSON after the edits
...
- `lib/Settings/openbuilt_register.json` parses as valid JSON.
