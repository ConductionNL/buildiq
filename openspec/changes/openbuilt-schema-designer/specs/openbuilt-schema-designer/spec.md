...
The schema designer provides a visual, guided authoring surface that lets
citizen developers design and evolve the data model of their virtual app
without writing JSON or deploying PHP. It manages schemas — JSON Schema
objects with `x-openregister-*` declarative extensions — in the virtual app's
per-version OR register (`openbuilt-{appSlug}-{versionSlug}`) via OR's
runtime schema CRUD API, producing exclusively declarative output per ADR-031.
The designer is the primary authoring tool for all declarative behaviour a
virtual app can express: lifecycle state machines, computed aggregations,
derived calculations, notification triggers, cross-schema relations, and
...
#### Scenario: REQ-OBSD-001-S1 Designer lists the schemas of the current virtual app

- **GIVEN** the virtual app `hello-world` has an active version `development`
  whose OR register is `openbuilt-hello-world-development`
- **WHEN** an authenticated user navigates to
  `/index.php/apps/openbuilt/builder/hello-world/schemas`
- **AND** that register contains a `hello-message` schema
- **THEN** the schema list panel renders one row for `hello-message`
- **AND** the row shows the slug, title, version, and property count

#### Scenario: REQ-OBSD-001-S2 Schemas from other virtual apps are not listed

- **GIVEN** a different virtual app's register contains a `customer` schema
- **WHEN** an authenticated user navigates to
  `/index.php/apps/openbuilt/builder/hello-world/schemas`
...
#### Scenario: REQ-OBSD-002-S1 Valid Add Schema submission persists and routes to the schema

- **GIVEN** no `customer` schema exists in the virtual app's register
- **WHEN** the user submits the Add Schema form with
  `slug: customer`, `title: "Klant"`, `version: 0.1.0`
...
#### Scenario: REQ-OBSD-002-S2 Duplicate slug is rejected before route change

- **GIVEN** a `customer` schema already exists in the namespace
- **WHEN** the user submits the Add Schema form with slug `customer`
...
- Reorder via drag-handle (the new order MUST be reflected in the
  saved JSON Schema's property order). Keyboard-accessible `↑`/`↓`
  buttons are provided as an alternative to drag-and-drop.
...
#### Scenario: REQ-OBSD-003-S1 Adding a string property with email format

- **GIVEN** the user is editing a schema in detail view
...
#### Scenario: REQ-OBSD-003-S2 Reordering two properties is reflected on reload

- **GIVEN** a schema with properties `title` and `body` in that order
...
- Authoring `on_transition` **actions** per transition chosen from a
  fixed typed enum matching OR's declarative action vocabulary:
  `audit-event-emit`, `notification-send`, `related-object-upsert`,
  `related-object-archive`, `webhook-dispatch`.
...
#### Scenario: REQ-OBSD-004-S1 Author a draft → published → archived lifecycle

- **GIVEN** the user is editing a schema's lifecycle
- **WHEN** the user adds three states (`draft`, `published`, `archived`)
  and three transitions (`draft → published`, `published → archived`,
  `archived → draft`)
...
#### Scenario: REQ-OBSD-004-S2 Removing the only initial-state designation surfaces a validation error

- **GIVEN** a lifecycle with exactly one state marked `initial`
- **WHEN** the user removes the `initial` designation from that state
...
- `AggregationEditor.vue` — adds entries to
  ...
  The filter expression SHALL be validated by `@openregister/declarative-dsl`;
  free-text PHP / JS SHALL be rejected with an inline error.
- `CalculationEditor.vue` — adds entries to
  ...
  vocabulary recognised by OR's calculation engine (ADR-031), validated
  by `@openregister/declarative-dsl`.
...
- `RelationEditor.vue` — adds entries to `x-openregister-relations`.
  Each entry is a typed record: `{ name, target (schema slug picker
  sourced from useSchemasStore.list), cardinality (one | many),
  inverse_of (optional property name on the target schema) }`.
- `WidgetEditor.vue` — adds entries to `x-openregister-widgets`. Each
  ...
  page editor (chain spec `openbuilt-page-designer`) can render the
  widget without re-authoring metadata.
...
#### Scenario: REQ-OBSD-005-S1 Add a count aggregation over a related collection

- **GIVEN** a `klant` schema with a related `bestelling` collection
- **WHEN** the user opens `AggregationEditor` on the `klant` schema
- **AND** adds an entry
  `{ name: "open_bestellingen", operation: "count", source: "bestelling", filter: "status != 'afgehandeld'" }`
- **AND** clicks Save
- **THEN** the persisted schema contains
  `x-openregister-aggregations.open_bestellingen` with the typed record
  exactly as authored
- **AND** OR's declarative engine reloads the schema and exposes
  `open_bestellingen` as a computed property

#### Scenario: REQ-OBSD-005-S2 Calculation editor rejects free-text PHP

- **GIVEN** the user is authoring a calculation expression
- **WHEN** the user types `<?php return $this->prijs * 1.21; ?>` into a
...
cache invalidation (per chain spec `openregister-runtime-schema-api`);
the designer SHALL NOT duplicate that work.

#### Scenario: REQ-OBSD-006-S1 Invalid staged state disables Save until corrected

- **GIVEN** a schema with a lifecycle that has exactly one initial state
- **WHEN** the user removes the `initial` designation from that state
- **THEN** the designer's live validator surfaces the error inline
- **AND** the **Save** button is disabled

#### Scenario: REQ-OBSD-006-S2 Successful save round-trips through the runtime endpoint

- **GIVEN** the user has a valid staged schema
...
A CI grep gate SHALL be enforced on every apply PR for this spec:
any occurrence of `eval(`, `<?php`, `Function(`, `script:`, `cb:`,
`handler:`, or `phpClass:` in `src/components/schema-designer/**` or
`src/views/SchemaDesigner.vue` is a blocking review finding.

#### Scenario: REQ-OBSD-007-S1 Designer output contains no imperative references

- **GIVEN** any schema authored through the designer
...
- **AND** every `x-openregister-*` block validates against the
  declarative-vocabulary JSON Schema published by OR (chain spec
  `openregister-runtime-schema-api`)
...
- **Delete property** — a `FieldRow.vue` remove action. `DeletePropertyDialog.vue`
  ...
  an explicit second click to confirm. The dialog lives in
  `src/dialogs/DeletePropertyDialog.vue` per ADR-004.
- **Delete schema** — a `SchemaListPanel.vue` per-row delete action.
  `DeleteSchemaDialog.vue` SHALL warn that all objects of this schema
  may be affected, and SHALL require the user to type the schema slug
  into a confirmation field before the **Delete** button activates.
  The dialog lives in `src/dialogs/DeleteSchemaDialog.vue` per ADR-004.

In both cases, cancelling the dialog SHALL leave the staged store
state unchanged.

#### Scenario: REQ-OBSD-008-S1 Delete-field requires a confirmation click

- **GIVEN** the user is viewing a schema's field list
- **WHEN** the user clicks the remove action on a `FieldRow.vue`
- **THEN** `DeletePropertyDialog.vue` renders
- **AND** the property is NOT removed from the staged store until the
  user clicks the dialog's **Confirm** button

#### Scenario: REQ-OBSD-008-S2 Delete-schema requires typing the slug

- **GIVEN** a schema with slug `klant` in the list panel
- **WHEN** the user clicks the delete action on the `klant` row
- **AND** `DeleteSchemaDialog.vue` renders with a typed-slug
  confirmation input
- **THEN** the dialog's **Delete** button is disabled until the user
  types `klant` exactly
- **AND** cancelling the dialog leaves the `klant` schema in the list
