...
chain:
  ...
  - openbuilt-template-catalogue  # THIS spec
---
...
manifest contract (chain #1); the template catalogue is the missing
"on-ramp" that closes the loop. This spec is the right place in the chain
...
The catalogue also exercises a foundational architectural commitment:
...
- **NEW** OR schema `ApplicationTemplate` declared in
  ...
  version }`. No bespoke state machine (ADR-031).
...
- **NEW** Frontend view `src/views/TemplateGallery.vue` — gallery
  ...
  free-text search over `title` + `useCase` + `description`.
...
- **New code** — `lib/Controller/ApplicationsController.php` adds one
  ...
  diff-by-diff. Screenshot PNGs for the four seeded templates ship in
...
- **OpenRegister** — adds one new schema (`ApplicationTemplate`) to
  ...
  `isSeeded:true` flag for the Conduction-shipped four.
...
- **Template versioning** — re-cloning an updated template against an
  ...
  semantics per REQ-OBTC-007). A future versioning spec can address
  propagating upstream improvements.
- **Files-API screenshot uploads** — screenshots in v1 ship in
  `img/templates/` committed to the repo. A follow-up spec can move
  these to OR Files for community-submitted templates.
- **US-5 (client intake app)** — depends on manifest-level
  OpenConnector binding metadata not yet in v1.4.x; deferred to a
  follow-up once the manifest gains OpenConnector binding metadata.
