...
- **GIVEN** the `openbuilt` register has the `ApplicationTemplate`
  schema declared
...
- **GIVEN** an `ApplicationTemplate` with `slug: permit-tracker`
  already exists in organisation `gemeente-amsterdam`
- **WHEN** a second `ApplicationTemplate` with the same `slug:
  permit-tracker` is submitted for the same organisation
...
- **GIVEN** a fresh Nextcloud instance with OpenBuilt installed
- **WHEN** the `SeedApplicationTemplates` repair step runs for the
  first time
...
- **GIVEN** a Nextcloud instance with OpenBuilt already installed and
  all four templates seeded
...
- **GIVEN** a user is on `/index.php/apps/openbuilt/templates` with
  all four seeded templates visible
- **WHEN** the user selects the `government-services` category filter
...
- **GIVEN** a user with no Applications in their organisation
- **WHEN** the user navigates to the OpenBuilt shell home
...
- **GIVEN** an authenticated user in organisation `gemeente-amsterdam`
- **WHEN** the user POSTs `{ name: "Mijn vergunningen", slug:
  "mijn-vergunningen" }` to
...
- **AND** a new `Application` record exists in OR with `slug:
  mijn-vergunningen`, `status: draft`, and a `manifest` equal to the
...
- **GIVEN** an `Application` with `slug: mijn-vergunningen` already
  exists in the calling user's organisation
- **WHEN** an authenticated user POSTs a clone with `slug:
  mijn-vergunningen`
...
`slug: mijn-vergunningen` SHALL produce a cloned schema with `slug:
mijn-vergunningen-permit-application`. The cloned manifest's page
`config` references to that schema SHALL be rewritten to the prefixed
slug so that the new Application loads correctly without manual edits.
...
- **GIVEN** an authenticated user in organisation `veiligheidsregio-brabant`
- **WHEN** the user clones `permit-tracker` into `slug:
  mijn-vergunningen` and then again into `slug: vggm-vergunningen`
- **THEN** OR contains two distinct schemas with slugs
  `mijn-vergunningen-permit-application` and
  `vggm-vergunningen-permit-application`
...
- **GIVEN** an authenticated user who has just successfully cloned
  `permit-tracker` from the gallery
- **WHEN** the clone response with 201 is received by the frontend
...
- **GIVEN** a user has cloned the `permit-tracker` template (version
  1.0.0) into a new Application `mijn-vergunningen`
- **WHEN** an admin updates the `permit-tracker` template's manifest
  in place (or the seed step re-runs against a new repo version with
  a bumped manifest)
...
- **GIVEN** a user is viewing the template gallery
- **WHEN** the user views the `permit-tracker` template card
  (which has `isSeeded: true`)
- **THEN** no "Edit template" or "Delete template" control is
  rendered on that card
...
- **GIVEN** a developer has modified the `permit-tracker` seed
  manifest to reference an unknown `type: xyz` page type
- **WHEN** the repair step runs on `occ maintenance:repair`
...
- **GIVEN** an authenticated user with the `nl_NL` locale configured
  in their Nextcloud profile
- **WHEN** the user opens `/index.php/apps/openbuilt/templates`
