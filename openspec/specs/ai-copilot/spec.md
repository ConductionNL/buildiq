...
The prompt-to-app surface: a natural-language brief is turned into a
...
degrades to fully absent (no button, no panel) when no provider is
configured. Two entry points share this contract: the creation wizard's
