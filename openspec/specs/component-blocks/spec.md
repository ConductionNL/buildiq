# component-blocks Specification

**OpenSpec changes**: [component-blocks](../../changes/archive/2026-07-24-component-blocks/) _(archived 2026-07-24)_
...
insert a block onto any page they have editor rights on. Insert always
deep-copies the fragment and mints fresh widget ids, so two insertions of the
same block never collide and editing the source block afterward never
changes an already-inserted copy. When a block's schema dependencies don't
exact-match the target app's schemas, insert opens an explicit remap
prompt — never a silent guess, never a silently dropped binding. Blocks
export/import as standalone JSON and appear in the template-catalogue
gallery under a distinct "Blocks" filter.
