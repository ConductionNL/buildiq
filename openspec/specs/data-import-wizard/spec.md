...
## Purpose
TBD - created by archiving change buildiq-data-import-wizard. Update Purpose after archive.
## Requirements
### Requirement: A maker can seed a virtual app's data from an uploaded file

Buildiq MUST offer a guided "Import data" wizard that lets a maker upload an
...
included) — Buildiq MUST NOT parse files, infer schemas, or write objects itself (ADR-022,
...
- **THEN** Buildiq MUST POST the file to OpenRegister's register import for the target register (no Buildiq-side parsing)
...
(`buildiq-{slug}-{versionSlug}`), never a shared/bound data register (those are read-only
...
(Buildiq's existing permission gate), and the import itself MUST be independently
re-checked server-side by OpenRegister's own manage-permission gate on the target register,
so Buildiq never becomes a permission-bypass path into OpenRegister data.
...
@e2e exclude double-authorisation covered by the Buildiq permission-gate unit test + OpenRegister's own import-auth test.
