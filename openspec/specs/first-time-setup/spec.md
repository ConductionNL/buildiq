...
**OpenSpec changes**: [openbuild-first-time-setup](../../changes/openbuild-first-time-setup/)

## Purpose

The ADR-042 first-time setup wizard for OpenBuild: the manifest `setup` block, the
admin-only idempotent `seed-templates` action behind it, and the completion stamp that
gates both re-display of the wizard and the `openbuild:getting-started` walkthrough.

Promoted verbatim from the `openbuild-first-time-setup` change delta so `@spec` anchors
have a canonical home. Anchors pointing into `openspec/changes/**` dangle the moment the
change is archived — which is exactly what had happened: all six anchors in
`SetupController` targeted `openbuild-first-time-setup/tasks.md#task-21|31|41`, and
`tasks.md` has no such headings (its headings are `## 2. Setup action endpoint` and
friends). gate-46 (spec-anchor-existence) flagged all six.
