# public-form-access Specification
...
OpenBuild's first anonymous (`#[PublicPage]`) surface: a `ShareToken` model
scoping public access to exactly one Application + one page, never a whole
Application. A token names a page and a mode (`submit` — anonymous create
form, `read` — public read-only page, `edit` — per-record edit link bound to
one object) and is managed from a `ShareTokenDialog` reachable from the page
designer toolbar and `AppSettingsModal`. Public rendering resolves solely
through the token (never session/organisation authorization — see
`openbuild-runtime`'s "Public manifest resolution never uses
session/organisation authorization" requirement) and returns a manifest
fragment containing only the bound page. Anonymous writes go through a
server-side service acting with the Application owner's authorization
context — never the OR client-facing objects API and never a visitor
identity, because none exists. A per-token honeypot field and
`#[AnonRateLimit]` guard both the render and submission routes.
