...
prefix is Buildiq's reserved-namespace marker, avoiding collision with
...
on `GET /index.php/apps/buildiq/api/applications/{slug}/manifest`. The underscore
prefix is Buildiq's system-reserved namespace marker — it prevents collision with
...
is provided, the register name SHALL be `buildiq-{appSlug}-{versionSlug}` (matching
...
- **THEN** the OR call targets register `buildiq-hello-world-staging`

#### Scenario: Store targets the production register when versionSlug is absent

- **GIVEN** the store's `versionSlug` is not set
- **AND** `Application.productionVersion.register` is `buildiq-hello-world-production`
- **WHEN** the store fetches schemas
- **THEN** the OR call targets register `buildiq-hello-world-production`
