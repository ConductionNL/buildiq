...
if (distinct[0] !== FLEET_DEXIE) {
	console.error(`x dexie singleton: built chunks carry dexie ${distinct[0]} but the fleet ships ${FLEET_DEXIE}. Buildiq's pages also carry hermiq's panel and openregister's integration script; two versions in one page blank them all.`)
	process.exit(1)
}
