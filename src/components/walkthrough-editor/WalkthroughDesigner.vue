	...
	<div class="wt-designer">
		<div class="wt-designer__bar">
			...
			<NcCheckboxRadioSwitch v-else
				...
				@update:model-value="setSetupEnabled">
				{{ t('openbuild', 'Enabled') }}
			</NcCheckboxRadioSwitch>
			...
			<ol class="wt-designer__steps">
				<li v-for="(step, si) in setupSteps" :key="step.id || si" class="wt-designer__step">
					<div class="wt-designer__step-head">
						...
						<NcTextField :label="t('openbuild', 'Step id')" :model-value="step.id || ''" @update:model-value="v => setSetupStep(si, 'id', v)" />
					...
					<div class="wt-designer__step-grid">
						<NcSelect :input-label="t('openbuild', 'Type')"
							...
							@update:model-value="v => setSetupStep(si, 'type', v)" />
						<NcTextField :label="t('openbuild', 'Title')" :model-value="step.title || ''" @update:model-value="v => setSetupStep(si, 'title', v)" />
						<NcTextField :label="t('openbuild', 'Body')" :model-value="step.body || ''" @update:model-value="v => setSetupStep(si, 'body', v)" />
						<NcTextField v-if="step.type === 'choice' || step.type === 'config-fields'"
							:label="t('openbuild', 'Config key')"
							:model-value="step.configKey || ''"
							@update:model-value="v => setSetupStep(si, 'configKey', v)" />
						<NcTextField v-if="step.type === 'run-action'"
							:label="t('openbuild', 'Action id')"
							:model-value="step.action || ''"
							@update:model-value="v => setSetupStep(si, 'action', v)" />
						<NcTextField v-if="step.type === 'component'"
							:label="t('openbuild', 'Component')"
							:model-value="step.component || ''"
							@update:model-value="v => setSetupStep(si, 'component', v)" />
					</div>
					<div v-if="step.type === 'choice'" class="wt-designer__options">
						...
						<div v-for="(opt, oi) in (step.options || [])" :key="oi" class="wt-designer__option-row">
							<NcTextField :label="t('openbuild', 'Value')" :model-value="opt.value != null ? String(opt.value) : ''" @update:model-value="v => setSetupOption(si, oi, 'value', v)" />
							<NcTextField :label="t('openbuild', 'Label')" :model-value="opt.label || ''" @update:model-value="v => setSetupOption(si, oi, 'label', v)" />
					...
					<div class="wt-designer__step-switches">
						<NcCheckboxRadioSwitch type="switch" :model-value="step.required === true" @update:model-value="v => setSetupStep(si, 'required', v)">
							{{ t('openbuild', 'Required (gates the app)') }}
						</NcCheckboxRadioSwitch>
						<NcCheckboxRadioSwitch v-if="step.type === 'choice'"
							type="switch"
							:model-value="step.multiple === true"
							@update:model-value="v => setSetupStep(si, 'multiple', v)">
							{{ t('openbuild', 'Multi-select') }}
						</NcCheckboxRadioSwitch>
						<NcCheckboxRadioSwitch v-if="step.type === 'summary'"
							type="switch"
							:model-value="step.healthCheck === true"
							@update:model-value="v => setSetupStep(si, 'healthCheck', v)">
		...
		<div v-else class="wt-designer__body">
			...
			<section v-if="activeTour" class="wt-designer__main">
				<div class="wt-designer__fields">
					<NcTextField :label="t('openbuild', 'Tour id')" :model-value="activeTour.id || ''" @update:model-value="v => setTour('id', v)" />
					<NcTextField :label="t('openbuild', 'Title')" :model-value="activeTour.title || ''" @update:model-value="v => setTour('title', v)" />
					<NcSelect :input-label="t('openbuild', 'Trigger')"
						:options="TRIGGERS"
						:model-value="activeTour.trigger || 'manual'"
						@update:model-value="v => setTour('trigger', v)" />
					<NcTextField :label="t('openbuild', 'Min app version')" :model-value="activeTour.minAppVersion || ''" @update:model-value="v => setTour('minAppVersion', v)" />
				...
				<ol class="wt-designer__steps">
					<li v-for="(step, si) in (activeTour.steps || [])" :key="step.id || si" class="wt-designer__step">
						<div class="wt-designer__step-head">
							<span class="wt-designer__step-num">{{ si + 1 }}</span>
							<NcTextField :label="t('openbuild', 'Step id')" :model-value="step.id || ''" @update:model-value="v => setStep(si, 'id', v)" />
							<NcButton type="tertiary"
								:disabled="si === 0"
								:aria-label="t('openbuild', 'Move up')"
								@click="moveStep(si, -1)">
						...
						<div class="wt-designer__step-grid">
							<NcTextField :label="t('openbuild', 'Title')" :model-value="step.title || ''" @update:model-value="v => setStep(si, 'title', v)" />
							<NcTextField :label="t('openbuild', 'Body')" :model-value="step.body || ''" @update:model-value="v => setStep(si, 'body', v)" />
							<NcTextField :label="t('openbuild', 'Task (optional)')" :model-value="step.task || ''" @update:model-value="v => setStep(si, 'task', v)" />
							<NcTextField :label="t('openbuild', 'Since version')" :model-value="step.sinceVersion || ''" @update:model-value="v => setStep(si, 'sinceVersion', v)" />
							<NcSelect :input-label="t('openbuild', 'Placement')"
								:options="PLACEMENTS"
								:model-value="step.placement || 'auto'"
								@update:model-value="v => setStep(si, 'placement', v)" />
							<NcSelect :input-label="t('openbuild', 'Target kind')"
								:options="TARGET_KINDS"
								:model-value="(step.target && step.target.kind) || 'nav-item'"
								@update:model-value="v => setTarget(si, 'kind', v)" />
							<NcTextField :label="t('openbuild', 'Target ref (route / widgetKey / id)')" :model-value="(step.target && step.target.ref) || ''" @update:model-value="v => setTarget(si, 'ref', v)" />
							<NcSelect :input-label="t('openbuild', 'Advance on')"
								:options="ADVANCE_TYPES"
								:model-value="(step.advanceOn && step.advanceOn.type) || 'manual'"
								@update:model-value="v => setAdvance(si, 'type', v)" />
							<NcTextField v-if="(step.advanceOn || {}).type === 'route-match'"
								:label="t('openbuild', 'Route')"
								:model-value="(step.advanceOn && step.advanceOn.route) || ''"
								@update:model-value="v => setAdvance(si, 'route', v)" />
							<template v-if="(step.advanceOn || {}).type === 'object-created'">
								<NcTextField :label="t('openbuild', 'Register')" :model-value="(step.advanceOn && step.advanceOn.register) || ''" @update:model-value="v => setAdvance(si, 'register', v)" />
								<NcTextField :label="t('openbuild', 'Schema')" :model-value="(step.advanceOn && step.advanceOn.schema) || ''" @update:model-value="v => setAdvance(si, 'schema', v)" />
							</template>
						</div>
						<div class="wt-designer__step-switches">
							<NcCheckboxRadioSwitch type="switch" :model-value="step.optional === true" @update:model-value="v => setStep(si, 'optional', v)">
								{{ t('openbuild', 'Optional (skip if absent)') }}
							</NcCheckboxRadioSwitch>
							<NcCheckboxRadioSwitch type="switch" :model-value="step.allowManualNext === true" @update:model-value="v => setStep(si, 'allowManualNext', v)">
