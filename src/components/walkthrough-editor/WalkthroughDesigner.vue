			...
			<ol class="wt-designer__steps">
				<li v-for="(step, si) in setupSteps" :key="step.id || si" class="wt-designer__step">
					<div class="wt-designer__step-head">
						...
						<NcTextField :label="t('openbuild', 'Step id')" :model-value="step.id || ''" @update:modelValue="v => setSetupStep(si, 'id', v)" />
					...
					<div v-if="step.type === 'choice'" class="wt-designer__options">
						...
						<div v-for="(opt, oi) in (step.options || [])" :key="oi" class="wt-designer__option-row">
							<NcTextField :label="t('openbuild', 'Value')" :model-value="opt.value != null ? String(opt.value) : ''" @update:modelValue="v => setSetupOption(si, oi, 'value', v)" />
							<NcTextField :label="t('openbuild', 'Label')" :model-value="opt.label || ''" @update:modelValue="v => setSetupOption(si, oi, 'label', v)" />
		...
		<div v-else class="wt-designer__body">
			...
			<section v-if="activeTour" class="wt-designer__main">
				<div class="wt-designer__fields">
					<NcTextField :label="t('openbuild', 'Tour id')" :model-value="activeTour.id || ''" @update:modelValue="v => setTour('id', v)" />
					<NcTextField :label="t('openbuild', 'Title')" :model-value="activeTour.title || ''" @update:modelValue="v => setTour('title', v)" />
				...
				<ol class="wt-designer__steps">
					<li v-for="(step, si) in (activeTour.steps || [])" :key="step.id || si" class="wt-designer__step">
						<div class="wt-designer__step-head">
							<span class="wt-designer__step-num">{{ si + 1 }}</span>
							<NcTextField :label="t('openbuild', 'Step id')" :model-value="step.id || ''" @update:modelValue="v => setStep(si, 'id', v)" />
							<NcButton type="tertiary"
								:disabled="si === 0"
								:aria-label="t('openbuild', 'Move up')"
								@click="moveStep(si, -1)">
						...
						<div class="wt-designer__step-grid">
							<NcTextField :label="t('openbuild', 'Title')" :model-value="step.title || ''" @update:modelValue="v => setStep(si, 'title', v)" />
							<NcTextField :label="t('openbuild', 'Body')" :model-value="step.body || ''" @update:modelValue="v => setStep(si, 'body', v)" />
							<NcTextField :label="t('openbuild', 'Task (optional)')" :model-value="step.task || ''" @update:modelValue="v => setStep(si, 'task', v)" />
							<NcTextField :label="t('openbuild', 'Since version')" :model-value="step.sinceVersion || ''" @update:modelValue="v => setStep(si, 'sinceVersion', v)" />
							...
							<NcTextField v-if="(step.advanceOn || {}).type === 'route-match'"
								...
								@update:modelValue="v => setAdvance(si, 'route', v)" />
							<template v-if="(step.advanceOn || {}).type === 'object-created'">
								<NcTextField :label="t('openbuild', 'Register')" :model-value="(step.advanceOn && step.advanceOn.register) || ''" @update:modelValue="v => setAdvance(si, 'register', v)" />
								<NcTextField :label="t('openbuild', 'Schema')" :model-value="(step.advanceOn && step.advanceOn.schema) || ''" @update:modelValue="v => setAdvance(si, 'schema', v)" />
							</template>
						</div>
						<div class="wt-designer__step-switches">
							<NcCheckboxRadioSwitch type="switch" :model-value="step.optional === true" @update:modelValue="v => setStep(si, 'optional', v)">
								{{ t('openbuild', 'Optional (skip if absent)') }}
							</NcCheckboxRadioSwitch>
							<NcCheckboxRadioSwitch type="switch" :model-value="step.allowManualNext === true" @update:modelValue="v => setStep(si, 'allowManualNext', v)">
