 ...
 * (e.g. `openconnector`) installed and enabled on this instance?
 *
 * The builder uses this to degrade gracefully (REQ-OCAS-005): when the app
 * is absent the connector origin option is disabled with a hint, but a
 * manual "enter endpoint path" escape hatch still allows authoring an
 ...
 * server-injected `OC.appswebroots` map, when available, is consulted first
 * as a synchronous positive signal. The result is cached per app id for the
 * session.
 *
 ...
 *
 * @param {string} appId - the app id, e.g. `openconnector`.
 * @param {object} [opts] - options.
 * @param {string} [opts.probePath] - app route to probe when the capability
...
export function useAppStatus(appId, opts = {}) {
	...
	/**
	 * Run the capability + probe check.
	 *
