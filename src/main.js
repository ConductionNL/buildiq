 ...
 *
 * ⚠️ `generateUrl('/apps/buildiq')` alone is not enough. Nextcloud serves the
 * app under BOTH `/apps/buildiq/...` and `/index.php/apps/buildiq/...`, but
 ...
 */
function routerBase() {
	const match = window.location.pathname.match(/^(.*\/apps\/buildiq)(?:\/|$)/)
	return match ? match[1] : generateUrl('/apps/buildiq')
}
