 ...
 * openbuild needs, surfaced through the `useManifestValidator` pipeline (the
...
export function validateExternalForms(manifest) {
	...
	if (!Array.isArray(externalForms)) {
		errors.push('/runtime/externalForms: openbuild.externalForm.error.not-array')
		return errors
	}
	...
	externalForms.forEach((entry, idx) => {
		...
		if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
			errors.push(at('openbuild.externalForm.error.invalid-shape'))
			return
		}
		...
		if (typeof entry.id !== 'string' || entry.id.trim() === '') {
			errors.push(at('openbuild.externalForm.error.id-required'))
		} else {
			if (seenIds.has(entry.id)) {
				errors.push(at('openbuild.externalForm.error.duplicate-id'))
			}
			seenIds.set(entry.id, idx)
		}
		// pageId — required.
		if (typeof entry.pageId !== 'string' || entry.pageId.trim() === '') {
			errors.push(at('openbuild.externalForm.error.page-id-required'))
		}
		// register — required.
		if (typeof entry.register !== 'string' || entry.register.trim() === '') {
			errors.push(at('openbuild.externalForm.error.register-required'))
		}
		// schema — required.
		if (typeof entry.schema !== 'string' || entry.schema.trim() === '') {
			errors.push(at('openbuild.externalForm.error.schema-required'))
		}
		// status — closed enum.
		...
		if (entry.portalPage !== undefined && entry.portalPage !== null) {
			...
			if (typeof pp !== 'object' || Array.isArray(pp)) {
				errors.push(at('openbuild.externalForm.error.portal-page-invalid'))
			} else {
