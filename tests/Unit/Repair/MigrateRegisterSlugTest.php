...
use OCA\Buildiq\Repair\MigrateRegisterSlug;
use OCA\Buildiq\Repair\MigrateRegisterSlugDecisions;
use OCP\DB\IResult;
use OCP\IAppConfig;
 ...
 *
 * @covers \OCA\Buildiq\Repair\MigrateRegisterSlug
 * @covers \OCA\Buildiq\Repair\MigrateRegisterSlugDecisions
 *
 * @spec exclude No canonical spec covers the `openbuild` -> `buildiq` register-slug
...
final class MigrateRegisterSlugTest extends TestCase {
	...
	public function testRenamesARegisterThatIsPresent(): void {
		$db = $this->dbReturning([['slug' => 'openbuild']]);
		...
		self::assertSame([['buildiq', 'openbuild']], $written);
	...
	public function testIsIdempotent(): void {
		$db = $this->dbReturning([['slug' => 'buildiq']]);
	...
	public function testRefusesWhenTheTargetSlugAlreadyExists(): void {
		$db = $this->dbReturning([['slug' => 'openbuild'], ['slug' => 'buildiq']]);
		$db->expects(self::never())->method('executeStatement');
	...
	public function testSlugsToReadCoversBothSidesOfTheMap(): void {
		...
		self::assertContains('openbuild', $slugs);
		self::assertContains('buildiq', $slugs);
	...
	public function testSlugsFromToleratesMissingAndNullSlugs(): void {
		self::assertSame(
			['openbuild', '', ''],
			$this->decisions->slugsFrom(rows: [['slug' => 'openbuild'], ['slug' => null], []])
		);
	...
	public function testAStoredConfigValueOnTheOldSlugIsRePointed(): void {
		...
		$step = new MigrateRegisterSlug(
			$db,
			$this->appConfig('openbuild', $written),
		...
		self::assertSame([['register', 'buildiq']], $written);
	...
	public function testAConfigWriteFailureIsLoggedNotThrown(): void {
		$cfg = $this->createMock(IAppConfig::class);
		$cfg->method('getValueString')->willReturn('openbuild');
