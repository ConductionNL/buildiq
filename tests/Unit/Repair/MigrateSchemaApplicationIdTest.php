...
use OCA\Buildiq\Repair\MigrateSchemaApplicationId;
use OCP\DB\IResult;
 ...
 *
 * @covers \OCA\Buildiq\Repair\MigrateSchemaApplicationId
 *
...
final class MigrateSchemaApplicationIdTest extends TestCase {
	...
	public function testMovesASchemaThatHasNoTwin(): void {
		...
		self::assertSame(
			[['buildiq', 'openbuild', 'case'], ['buildiq', 'openbuild', 'task']],
			$written
		);
	...
	public function testRefusesASlugThatWouldCollide(): void {
		...
		$output = $this->createMock(IOutput::class);
		$output->expects(self::once())->method('info')->with(self::stringContains('1 schema(s) moved to buildiq, 1 refused'));

		$step->run($output);

		self::assertSame([['buildiq', 'openbuild', 'task']], $written);
