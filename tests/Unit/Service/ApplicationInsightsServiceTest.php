...
class ApplicationInsightsServiceTest extends TestCase {
	...
	public function testHybridAppComputesFromInstalledAppRegisters(): void {
		...
		$reg = $this->getMockBuilder(Register::class)->disableOriginalConstructor()->onlyMethods(['getSchemas', 'getId'])->addMethods(['getApplication'])->getMock();
		$reg->method('getApplication')->willReturn('pipelinq');
		$reg->method('getId')->willReturn(16);
		$reg->method('getSchemas')->willReturn([16, 17]);

		$other = $this->getMockBuilder(Register::class)->disableOriginalConstructor()->onlyMethods(['getSchemas', 'getId'])->addMethods(['getApplication'])->getMock();
