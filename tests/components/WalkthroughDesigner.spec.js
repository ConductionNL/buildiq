...
describe('WalkthroughDesigner — setup-block editing (REQ-WALK-OB-005)', () => {
	function setupFactory(setup = { enabled: true, steps: [] }) {
		return shallowMount(WalkthroughDesigner, {
			propsData: { manifest: { version: '1.0.0', menu: [], pages: [], walkthrough: { enabled: true, tours: [] }, setup } },
			mocks: { t: (app, str) => str },
		})
	}
	...
	it('addSetupStep seeds an info step', () => {
		...
		w.vm.addSetupStep()
		expect(w.emitted('update:manifest').pop()[0].setup.steps[0].type).toBe('info')
	})
	...
	it('setSetupStep edits a field on an existing step', () => {
		...
		expect(w.emitted('update:manifest').pop()[0].setup.steps[0].type).toBe('choice')
	})

	it('addSetupOption creates an option row; setSetupOption edits it', () => {
		const w1 = setupFactory({ enabled: true, steps: [{ id: 'region', type: 'choice' }] })
		w1.vm.addSetupOption(0)
		expect(w1.emitted('update:manifest').pop()[0].setup.steps[0].options).toEqual([{ value: '', label: '' }])
		const w2 = setupFactory({ enabled: true, steps: [{ id: 'region', type: 'choice', options: [{ value: '', label: '' }] }] })
		w2.vm.setSetupOption(0, 0, 'value', 'nl')
		expect(w2.emitted('update:manifest').pop()[0].setup.steps[0].options[0].value).toBe('nl')
	})

	it('moveSetupStep reorders; deleteSetupStep removes', () => {
		const w = setupFactory({ enabled: true, steps: [{ id: 'a', type: 'info' }, { id: 'b', type: 'summary' }] })
		w.vm.moveSetupStep(0, 1)
		expect(w.emitted('update:manifest').pop()[0].setup.steps.map((s) => s.id)).toEqual(['b', 'a'])
		w.vm.deleteSetupStep(0)
		expect(w.emitted('update:manifest').pop()[0].setup.steps.map((s) => s.id)).toEqual(['b'])
	})
