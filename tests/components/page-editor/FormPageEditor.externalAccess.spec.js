...
vi.mock('../../../src/components/page-editor/fields/FormFieldBuilder.vue', () => ({
	default: { name: 'FormFieldBuilder', props: ['modelValue'], render(h) { return h('div') } },
}))
...
describe('FormPageEditor — External access section', () => {
	it('resolves register/schema from an OR-shaped submitEndpoint', () => {
		const wrapper = mountEditor({ config: { submitEndpoint: '/api/objects/intake/report' }, pageId: 'page-1' })
		expect(wrapper.vm.externalTarget).toEqual({ register: 'intake', schema: 'report' })
	})

	it('is null (disabled) for a handler-shaped form', () => {
		const wrapper = mountEditor({ config: { submitHandler: 'saveDraft' }, pageId: 'page-1' })
	...
	it('is null for a non-OR endpoint shape', () => {
		const wrapper = mountEditor({ config: { submitEndpoint: '/apps/custom/whatever' }, pageId: 'page-1' })
		expect(wrapper.vm.externalTarget).toBeNull()
	})
	...
	it('finds the existing runtime.externalForms entry for this pageId only', () => {
		const forms = [
			...
			{ id: 'ef-2', pageId: 'page-2', register: 'other', schema: 'x', status: 'enabled' },
		]
		const wrapper = mountEditor({ config: { submitEndpoint: '/api/objects/intake/report' }, pageId: 'page-1', runtimeExternalForms: forms })
	...
	it('onExternalFormSave appends a new entry when none exists for this page', async () => {
		const wrapper = mountEditor({ config: { submitEndpoint: '/api/objects/intake/report' }, pageId: 'page-1', runtimeExternalForms: [] })
		const entry = { id: 'ef-1', pageId: 'page-1', register: 'intake', schema: 'report', status: 'enabled' }
	...
	it('onExternalFormSave replaces the existing entry for this page (find-or-append)', async () => {
		const existing = [{ id: 'ef-1', pageId: 'page-1', register: 'intake', schema: 'report', status: 'enabled' }]
		const wrapper = mountEditor({ config: { submitEndpoint: '/api/objects/intake/report' }, pageId: 'page-1', runtimeExternalForms: existing })
		const updated = { id: 'ef-1', pageId: 'page-1', register: 'intake', schema: 'report', status: 'disabled' }
	...
	it('onExternalFormSave does not disturb another page\'s entry', async () => {
		const existing = [{ id: 'ef-2', pageId: 'page-2', register: 'other', schema: 'x', status: 'enabled' }]
		const wrapper = mountEditor({ config: { submitEndpoint: '/api/objects/intake/report' }, pageId: 'page-1', runtimeExternalForms: existing })
		const entry = { id: 'ef-1', pageId: 'page-1', register: 'intake', schema: 'report', status: 'enabled' }
		wrapper.vm.onExternalFormSave(entry)
		await wrapper.vm.$nextTick()
