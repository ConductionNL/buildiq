...
describe('FormPageEditor Steps fieldset', () => {
	it('mounts the Steps fieldset with a FormStepsManager', () => {
		...
		expect(wrapper.findComponent({ name: 'FormStepsManager' }).exists()).toBe(true)
	})
	...
	it('emitting null from FormStepsManager removes the steps key', async () => {
		const wrapper = mountEditor({ steps: [{ id: 'a', title: 'A', fields: ['x'] }] })
		const stepsManager = wrapper.findComponent({ name: 'FormStepsManager' })
	...
	it('markFor("steps") renders an inline mark when the injected validator reports a steps error', () => {
		const errorFor = (key) => (key === 'steps' ? { hasError: true, message: 'steps: incomplete partition' } : { hasError: false, message: '' })
		const wrapper = mountEditor({}, {
			pageEditorValidator: { register: () => {}, unregister: () => {}, errorFor },
		})
		const marks = wrapper.findAllComponents({ name: 'InlineFieldMark' })
		const stepsMark = marks.filter((m) => m.props('error') && m.props('error').hasError && m.props('error').message.includes('steps'))
