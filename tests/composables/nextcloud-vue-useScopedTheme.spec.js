...
import { useScopedTheme, SCOPE_ATTR, rewriteRootScope } from '@conduction/nextcloud-vue/dist/esm/composables/useScopedTheme.js'
...
describe('published @conduction/nextcloud-vue useScopedTheme (real dist, not the vitest stub)', () => {
	...
	it('is published at 1.0.0-beta.221 or later', () => {
		expect(pkg.version).toMatch(/^1\.0\.0-beta\.(22[1-9]|2[3-9]\d|[3-9]\d\d)$|^[2-9]\./)
	})
	...
	it('apply() fetches, rewrites, and injects exactly one scoped style element', async () => {
		const doc = fakeDoc()
		axiosMock.get.mockResolvedValue({ data: ':root {\n  --nldesign-color-primary: #004699;\n}\n' })
		const scopedTheme = useScopedTheme({ doc })
		const manifest = { runtime: { theme: { source: 'nldesign', tokenSet: 'amsterdam' } } }
		const injected = await scopedTheme.apply(manifest, 'petstore')
		expect(injected).toBe(true)
		expect(doc.head.children).toHaveLength(1)
		expect(doc.head.children[0].textContent).toContain('[data-nldesign-theme-scope="petstore"]')
	})
	...
	it('apply() degrades to default styling with a warning when the token asset is unreachable (REQ-NTS-003 "Missing token asset degrades to default styling")', async () => {
		...
		const manifest = { runtime: { theme: { source: 'nldesign', tokenSet: 'ghost' } } }
	...
	it('teardown() removes the managed element', async () => {
		...
		const scopedTheme = useScopedTheme({ doc })
		await scopedTheme.apply({ runtime: { theme: { source: 'nldesign', tokenSet: 'amsterdam-2' } } }, 'petstore-2')
	...
	it('listTokenSets() resolves the real endpoint shape and degrades to [] on failure', async () => {
		...
		axiosMock.get.mockResolvedValueOnce({ data: { tokenSets: [{ id: 'amsterdam', name: 'Amsterdam' }] } })
		expect(await scopedTheme.listTokenSets()).toEqual([{ id: 'amsterdam', name: 'Amsterdam' }])
	...
	it('evaluateContrast() resolves results and null (not []) on failure', async () => {
		const scopedTheme = useScopedTheme()
		axiosMock.post.mockResolvedValueOnce({ data: { results: [{ name: 'Primary', ratio: 4.6, level: 'AA', pass: true }] } })
		expect(await scopedTheme.evaluateContrast([{ name: 'Primary', value: '#004699', role: 'ui' }], '#FFFFFF'))
