 ...
 *
 * Buildiq ships NO concrete HealthController or MetricsController. Their route
 ...
 * app at a time — and `buildiq` sorts BEFORE `openregister`, so
 * `OCA\OpenRegister\` is not on the autoloader when Buildiq's own
 ...
 * What WAS measured failing on that same run is the CLI SAPI — `Buildiq:
...
test.describe('ADR-040 AppHost adoption', () => {
	test('serves /api/health from the AppHost generic controller', async ({
		...
		const response = await request.get('/index.php/apps/buildiq/api/health')
		...
		expect(
			...
			'health#index is bound only by Bootstrap::register(). A 500 here means '
				...
				+ "autoloadable during buildiq's register() — the ADR-040 prelude is "
	...
	test('serves /api/metrics from the AppHost generic controller', async ({
		...
		const response = await request.get('/index.php/apps/buildiq/api/metrics')
