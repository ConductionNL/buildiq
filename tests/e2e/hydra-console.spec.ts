...
async function openConsolePage(
	...
	await expect(
		page.getByRole('link', { name: 'Findings' }).or(page.getByText('Pipeline', { exact: true })).first(),
...
test.describe('hydra-console — live console over the hydra-cache register', () => {
	...
	test('Dashboard KPI cards paint the register\'s REAL counts', async ({ page, request }) => {
		...
		for (const [label, body] of Object.entries({ cycles, needsInput, openFindings, changes })) {
			expect(body.total, `seed precondition: ${label} must be > 0`).toBeGreaterThan(0)
		}

		await openConsolePage(page, '/', DASHBOARD_ONLY, true)

		const card = (caption: string) => page.locator('.cn-grid__item').filter({ hasText: caption })

		await expect(card('pipeline runs in the cache'))
			.toHaveText(new RegExp(`Cycles\\s*${cycles.total}\\s*pipeline runs in the cache`), { timeout: SLOW })
		await expect(card('escalated to a human'))
			.toHaveText(new RegExp(`Needs input\\s*${needsInput.total}\\s*escalated to a human`), { timeout: SLOW })
		await expect(card('still waiting on a fix'))
			.toHaveText(new RegExp(`Open findings\\s*${openFindings.total}\\s*still waiting on a fix`), { timeout: SLOW })
		await expect(card('specs the pipeline is tracking'))
			.toHaveText(new RegExp(`Changes\\s*${changes.total}\\s*specs the pipeline is tracking`), { timeout: SLOW })
	})

	test('Dashboard charts paint real facet buckets, not empty canvases', async ({ page, request }) => {
		// The gate facet is the bucket set behind "Findings by gate".
		const facet = await orGet(request, '/finding?_limit=1&_facets%5Bgate%5D%5Btype%5D=terms')
		const gateBuckets: Array<{ value: string }> = facet?.facets?.gate?.data?.buckets ?? []
		expect(gateBuckets.length, 'seed precondition: gate facet must have buckets').toBeGreaterThan(0)
		expect(
		...
		await expect(page.locator('.apexcharts-canvas')).toHaveCount(6, { timeout: SLOW })
		...
		for (const bucket of gateBuckets.map((b) => b.value)) {
			await expect(
				page.locator('.apexcharts-canvas').filter({ hasText: bucket }).first(),
				`the "Findings by gate" chart must render the "${bucket}" bucket`,
			).toBeVisible({ timeout: SLOW })
		}
		// Cross-chart spot checks against other schemas' facets.
		await expect(
			page.locator('.apexcharts-canvas').filter({ hasText: 'decidesk' }).first(),
			'the "Changes by app" chart must render the decidesk bucket',
		).toBeVisible({ timeout: SLOW })
		await expect(
			page.locator('.apexcharts-canvas').filter({ hasText: 'CRITICAL' }).first(),
	...
	test('Changes index lists the seeded change row', async ({ page, request }) => {
		...
		const seeded = changes.results.find((c: any) => c.slug === 'p2-agenda-management')
		expect(seeded, 'seed precondition: change p2-agenda-management must exist').toBeTruthy()
		...
		await expect(page.getByText(new RegExp(`Showing \\d+ of ${changes.total}`))).toBeVisible({ timeout: SLOW })
	})

	test('Findings index lists every seeded finding, including the spdx-headers CRITICAL', async ({ page, request }) => {
		const findings = await orGet(request, '/finding?_limit=50')
		expect(findings.total, 'seed precondition: findings must exist').toBeGreaterThan(0)

		await openConsolePage(page, '/findings', 'spdx-headers')

		await expect(page.getByText(new RegExp(`Showing \\d+ of ${findings.total}`))).toBeVisible({ timeout: SLOW })
		...
		await expect(page.getByRole('row')).toHaveCount(findings.total + 1, { timeout: SLOW })
	})

	test('Cycles index lists the seeded cycle with its real outcome', async ({ page, request }) => {
		const cycles = await orGet(request, '/cycle?_limit=20')
		expect(cycles.total, 'seed precondition: cycles must exist').toBeGreaterThan(0)

		await openConsolePage(page, '/cycles', /phpcs failures persisted after reviewer fixes/)

		const row = page.getByRole('row').filter({ hasText: 'phpcs failures persisted after reviewer fixes' })
		await expect(row).toHaveCount(1)
		await expect(row).toContainText(/needs-input/i)
		await expect(row).toContainText('3,603')
	})

	test('ChangeDetail renders its widget grid — data, related cycles and the audit-trail widget', async ({ page, request }) => {
		const changes = await orGet(request, '/change?_limit=20')
		const seeded = changes.results.find((c: any) => c.slug === 'p2-agenda-management')
		expect(seeded, 'seed precondition: change p2-agenda-management must exist').toBeTruthy()
		...
		const dataCard = page.locator('.cn-widget-wrapper').filter({ hasText: 'Application' })
		...
		const cyclesCard = page.locator('.cn-widget-wrapper').filter({ hasText: 'TRIGGER' })
	...
	test('ChangeDetail renders the three command buttons (render-only — openconnector#1068)', async ({ page, request }) => {
		...
		expect(seeded.pipelineState, 'seed precondition for the Re-dispatch visibleWhen guard').toBe('needs-input')

		await openConsolePage(page, `/changes/${seeded.id}`, 'p2-agenda-management')

		for (const label of ['Re-dispatch', 'Mark needs-input', 'Rebuild from scratch']) {
			const button = page.getByRole('button', { name: label, exact: true })
			await expect(button, `header action "${label}" must render`).toBeVisible({ timeout: SLOW })
			await expect(button).toBeEnabled()
		}
	})

	test('CycleDetail renders core data, the stage summary and the findings list', async ({ page, request }) => {
		const cycles = await orGet(request, '/cycle?_limit=20')
		const cycle = cycles.results[0]
		expect(cycle, 'seed precondition: a cycle must exist').toBeTruthy()

		await openConsolePage(page, `/cycles/${cycle.id}`, /phpcs failures persisted after reviewer fixes/)

		// `cycle-core` data widget.
		const coreCard = page.locator('.cn-widget-wrapper').filter({ hasText: 'Outcome reason' })
		...
		const stagesCard = page.locator('.cn-widget-wrapper').filter({ hasText: 'Stage summary' })
		...
		const cycleFindings = await orGet(request, `/finding?_limit=1&cycle=${cycle.id}`)
		expect(cycleFindings.total, 'seed precondition: the cycle must have findings').toBeGreaterThan(0)
		const findingsCard = page.locator('.cn-widget-wrapper').filter({ hasText: 'SEVERITY' })
		...
		await expect(page.getByRole('heading', { name: 'Audit trail' })).toBeVisible({ timeout: SLOW })
	})

	test('FindingDetail renders the seeded b1 finding', async ({ page, request }) => {
		const findings = await orGet(request, '/finding?_limit=50')
		const b1 = findings.results.find((f: any) => f.findingId === 'b1')
		expect(b1, 'seed precondition: finding b1 must exist').toBeTruthy()

		await openConsolePage(page, `/findings/${b1.id}`, 'missing SPDX-License-Identifier')

		const card = page.locator('.cn-widget-wrapper').filter({ hasText: 'Finding id' })
	...
	test('hermiq agent leaf widget paints nothing (hermiq#42 / #44)', async ({ page, request }) => {
	...
	test('audit-trail widget shows the object\'s real history', async ({ page, request }) => {
		...
		expect(trail.ok(), 'the PLURAL audit-trails endpoint must respond 2xx').toBeTruthy()
		const trailBody = await trail.json()
		expect(trailBody.results.length, 'the change must have audit entries').toBeGreaterThan(0)
	...
	test('detail widgets use their manifest-declared titles', async ({ page, request }) => {
