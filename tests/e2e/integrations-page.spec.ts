...
const CONNECTIONS_API = '/index.php/apps/openregister/api/objects/integriq/app_connection?app=buildiq&_limit=50'
...
async function storeRow(request: APIRequestContext): Promise<string> {
	const list = await request.get(CONNECTIONS_API, { headers: { Accept: 'application/json' } })
	const rows = list.ok() ? ((await list.json()).results ?? []) : []
	const store = rows.find((row: Record<string, unknown>) => row.key === 'store' && row.app === 'buildiq')
 ...
 */
async function withRegistryUrl(request: APIRequestContext, url: string, block: () => Promise<void>): Promise<void> {
	...
	try {
		const saved = await request.post(SETTINGS_API, { headers: HEADERS, data: { registry_url: url } })
		expect(saved.ok(), `settings save -> ${saved.status()}`).toBeTruthy()
		await block()
	} finally {
		await request.post(SETTINGS_API, { headers: HEADERS, data: { registry_url: previous } })
	}
}
...
test.describe('Integrations over the connection registry', () => {
	test('lists the four declared connections, all of them Buildiq\'s', async ({ page }) => {
		const byKey = await rowsByKey(page.request)
		expect(Object.keys(byKey).sort()).toEqual(DECLARED.map((d) => d.key).sort())
		expect(String(byKey.store?.settingsUrl ?? '')).toBe('/settings/admin/buildiq#section-store')
	...
	test('reads Configured once a registry URL is saved', async ({ page }) => {
		await withRegistryUrl(page.request, 'https://store.example.invalid/index.php', async () => {
			await expect
	...
	test('reads Error naming the host once a search cannot reach the store', async ({ page }) => {
		await withRegistryUrl(page.request, 'https://store.example.invalid/index.php', async () => {
			...
			const search = await page.request.get(STORE_SEARCH_API, { headers: HEADERS, failOnStatusCode: false })
			expect(search.status(), `store search -> ${search.status()}`).toBe(200)
			expect((await search.json()).outcome).toBe('store_unreachable')

			await expect
				.poll(() => storeRow(page.request), { timeout: 15_000 })
				.toBe('error The last search could not reach the template store at store.example.invalid.')
		})
	})
	...
	test('sends Add integration to integriq instead of offering a form', async ({ page }) => {
		...
		await Promise.all([
			page.waitForURL(/\/apps\/integriq\/connections\?app=buildiq&link=1$/, { timeout: 30_000 }),
