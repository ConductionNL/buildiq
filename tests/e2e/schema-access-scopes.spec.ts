...
async function getSchema(page: import('@playwright/test').Page, slug: string) {
	const resp = await page.request.get(
		`${BASE_URL}/index.php/apps/openregister/api/schemas/${slug}`,
		{ headers: { 'OCS-APIRequest': 'true' } },
	)
	expect(resp.ok(), `GET schema ${slug} must succeed`).toBeTruthy()
	return resp.json()
}
...
async function putSchema(page: import('@playwright/test').Page, slug: string, body: Record<string, unknown>) {
	...
	expect(resp.ok(), `PUT schema ${slug} must succeed`).toBeTruthy()
	return resp.json()
}
...
test.describe.skip('data-scopes-authoring — Access sub-editor (REQ-OBDSA-001/002/003/005)', () => {
	test.beforeEach(async ({ page }) => {
		// Step 1 — ensure the pw-access-scopes virtual app exists (idempotent
		// across runs — the createApplicationWizard/bootstrap-openbuild
		// specs establish the same "create if not present" convention).
		...
		if (await addAppButton.isVisible().catch(() => false)) {
			...
			await page.getByLabel(/title/i).fill('PW Access Scopes')
	...
	test('REQ-OBDSA-001 scenario 1: group read scope persists via Save → reload', async ({ page }) => {
		...
		// Add the schema if it doesn't already exist from a previous run.
		const existingRow = page.locator('.openbuild-schema-list__row').filter({ hasText: SCOPED_SCHEMA_SLUG })
		if ((await existingRow.count()) === 0) {
			await page.getByRole('button', { name: /add schema/i }).first().click()
			await page.getByLabel(/slug/i).fill(SCOPED_SCHEMA_SLUG)
			await page.getByLabel(/title/i).fill('Record')
		...
		await expect(readRow.getByText(/specific groups/i)).toBeVisible({ timeout: 10_000 })
	})

	test('REQ-OBDSA-001 scenario 2: independent per-operation scopes', async ({ page }) => {
		await page.goto(`${BASE_URL}/apps/openbuild/builder/${APP_SLUG}/schemas`, { waitUntil: 'domcontentloaded' })
		const row = page.locator('.openbuild-schema-list__row').filter({ hasText: SCOPED_SCHEMA_SLUG })
		await row.click()
		await expect(page.getByRole('button', { name: /back to schemas/i })).toBeVisible({ timeout: 10_000 })
	...
	test('REQ-OBDSA-002 scenario 1: unrelated field edit + save preserves an API-seeded authorization block', async ({ page }) => {
		...
		await page.goto(`${BASE_URL}/apps/openbuild/builder/${APP_SLUG}/schemas`, { waitUntil: 'domcontentloaded' })
		await page.locator('.openbuild-schema-list__row').filter({ hasText: SCOPED_SCHEMA_SLUG }).click()
		await expect(page.getByRole('button', { name: /back to schemas/i })).toBeVisible({ timeout: 10_000 })
	...
	test('REQ-OBDSA-002 scenario 2: an API-seeded @creator entry renders read-only and survives byte-identical after save', async ({ page }) => {
		...
		await expect(updateRow.getByText(/managed outside the designer/i)).toBeVisible({ timeout: 10_000 })
	...
	test('REQ-OBDSA-005: scoped schema shows a "Restricted" badge, unscoped schema shows none', async ({ page }) => {
		...
		const unscopedRow = page.locator('.openbuild-schema-list__row').filter({ hasText: UNSCOPED_SCHEMA_SLUG })
		...
		const scopedRow = page.locator('.openbuild-schema-list__row').filter({ hasText: SCOPED_SCHEMA_SLUG })
		const scopedBadge = scopedRow.locator('.openbuild-schema-list__badge')
		await expect(scopedBadge).toBeVisible({ timeout: 10_000 })
		await expect(scopedBadge).toHaveAttribute('title', /read: vets|update: @creator/)

		await expect(
			page.locator('.openbuild-schema-list__row').filter({ hasText: UNSCOPED_SCHEMA_SLUG })
