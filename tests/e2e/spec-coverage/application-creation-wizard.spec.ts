...
test('REQ-OBWIZ-002 — selecting Custom preset shows the custom-chain composer in step 3', async ({ page }) => {
	...
	const customPreset = modal.locator('.wizard-step2__preset-card').filter({ hasText: /custom/i }).first()
	await expect(customPreset, 'the Custom preset card must be present').toBeVisible({ timeout: 5_000 })
...
test('REQ-OBWIZ-004 — custom chain composer allows adding and reordering version rows', async ({ page }) => {
	...
	await expect(versionRows, 'the composer must seed one version row').toHaveCount(1)
	await addVersionBtn.click()
	await expect(versionRows, 'adding a row must extend the chain to two versions').toHaveCount(2, { timeout: 5_000 })
})
...
test('REQ-OBWIZ-004 — custom chain composer enforces minimum one version row', async ({ page }) => {
	...
	const versionRows = modal.locator('.wizard-step3__row')
	await expect(versionRows, 'the composer must seed exactly one version row').toHaveCount(1, { timeout: 5_000 })

	// Try to remove the only row — the composer must refuse and say why.
	await modal.locator('.wizard-step3__btn-remove').first().click()
	await expect(versionRows, 'the last remaining version row must not be removable').toHaveCount(1)
	await expect(
		modal.locator('.wizard-step3__error-msg').filter({ hasText: /at least one/i }).first(),
...
test('REQ-OBWIZ-005 — leading-underscore slug shows inline error and blocks advancing', async ({ page }) => {
	// @e2e application-creation-wizard::leading-underscore-slug-is-rejected
	test.skip(!LIVE, 'Requires live dev env with wizard built and accessible — set OPENBUILD_E2E_LIVE=1')
	...
	await expect(
		modal.locator('.wizard-step1__error-msg').filter({ hasText: /cannot start with/i }).first(),
	...
	await modal.locator('button').filter({ hasText: /next|continue/i }).first().click()
	await expect(modal.locator('.wizard-step1'), 'the wizard must stay on step 1').toBeVisible()
	await expect(modal.locator('.wizard-step2'), 'the wizard must not reach the preset step').toHaveCount(0)
})
...
test('REQ-OBWIZ-006 — duplicate version slugs in custom chain shows inline error', async ({ page }) => {
	...
	await expect(addVersionBtn).toBeVisible({ timeout: 5_000 })
	await addVersionBtn.click()
	await expect(modal.locator('.wizard-step3__row')).toHaveCount(2, { timeout: 5_000 })
	...
	await modal.locator('button').filter({ hasText: /next|continue/i }).first().click()
	await expect(modal.locator('.wizard-step3'), 'the wizard must stay on the chain step').toBeVisible()
	await expect(modal.locator('.wizard-step4'), 'the wizard must not reach Review').toHaveCount(0)
})
