...
test('REQ-OBTC-006 — an installable gallery card exposes the clone action and it opens the clone dialog', async ({ page }) => {
	// @e2e openbuild-template-catalogue::clone-redirects-into-the-page-editor
	...
	const installAction = page.locator('.template-card__actions button', { hasText: /^install$/i }).first()

	await expect
		.poll(async () => (await installAction.count()) > 0 || (await unavailableHint.count()) > 0, {
			message: 'the App store must render either installable cards or the documented GitHub-unavailable hint',
			timeout: 20_000,
		})
		.toBe(true)
	...
	if (await unavailableHint.count() > 0) {
		...
		await expect(unavailableHint, 'GitHub-unavailable hint must be visible').toBeVisible()
		return
	}

	// Happy path: the card-level clone action must open the clone dialog.
	await expect(installAction, 'an installable card must expose the clone action').toBeVisible()
	await installAction.click()
	const dialog = page.locator('.clone-dialog')
	await expect(dialog, 'the clone action must open CloneTemplateDialog').toBeVisible({ timeout: 10_000 })
	await expect(dialog.locator('#clone-template-dialog-title')).toHaveText(/install app from github|use this template|install template/i)
})
