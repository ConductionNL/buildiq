...
} from '../../src/services/manifestValidation/connectorDataSource.js'
...
describe('validateConnectorDataSource', () => {
	...
	it('ignores a dataSource with no connector branch', () => {
		expect(
			validateConnectorDataSource({ register: 'r', schema: 's' }, P),
		).toEqual([])
	})
	...
	it('rejects endpointPath with a scheme', () => {
		const ds = {
			connector: {
				endpointPath: 'https://evil.example/x',
				fields: { a: 'a' },
			},
		}
		const errs = validateConnectorDataSource(ds, P)
	...
	it('rejects an unsupported method', () => {
		const errs = validateConnectorDataSource(
			{ connector: { endpointPath: 'x', method: 'POST', fields: { a: 'a' } } },
			P,
		)
	...
	it('rejects empty fields', () => {
		const errs = validateConnectorDataSource(
			{ connector: { endpointPath: 'x', fields: {} } },
			P,
		)
	...
	it('rejects an invalid field selector', () => {
		const errs = validateConnectorDataSource(
			{ connector: { endpointPath: 'x', fields: { a: 'a..b' } } },
			P,
		)
	...
	it('rejects an unknown key', () => {
		const errs = validateConnectorDataSource(
			{ connector: { endpointPath: 'x', fields: { a: 'a' }, bogus: 1 } },
			P,
		)
	...
	it('walks pages and widgets', () => {
		const manifest = {
			pages: [
				{
					config: {
						dataSource: { connector: { endpointPath: 'x', fields: {} } },
					},
				},
				{
					config: {
		...
		expect(errs.some((e) => e.startsWith('/pages/0/config/dataSource'))).toBe(
			true,
		)
		expect(
			errs.some((e) => e.startsWith('/pages/1/config/widgets/0/dataSource')),
		).toBe(true)
	})
